...
3. 基于读到的全文填写 `Role` 中的 `Purpose`、`Responsibilities`、`Out of scope`、`Preferred working style` 等现有字段，也可按需新增角色字段或专用规则。结合已知用途写清目标、职责、边界和工作方式；信息不足时保留未知字段，不虚构用户偏好、资质或权限。保留基础模板的标题、字段名及 `Baseline Rules` 的全部基础规则，不删除、替换或用新增规则抵消基础规则，也不复制一份可能过期的模板。
4. `workspace update --content` 是完整覆盖：提交读回原文加角色编辑后的完整内容，不能只提交新字段或 Role 段落。补充、修改或重写既有 Agent 的行为模板时同样先读原文，按用户要求调整相应角色字段或专用规则，保留无关内容；重写角色描述也必须保留 `Baseline Rules` 标题及全部基础规则。
