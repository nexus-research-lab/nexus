...
补充、修改或重写已有 Agent 的角色职责与工作方式时，读取 [references/agent-profile-template.md](references/agent-profile-template.md)，按其流程编辑 workspace 根级 `AGENTS.md` 并读回核对；名称、头像、目录摘要与 runtime 配置使用 `agents` 配置域。
