...
2. 用 `nexus_runtime.command` 的 `goal/inspect` 确认当前会话没有未结束 Goal。
3. 按 Skill 的 contract → structured input → invoke 流程执行 `create_goal`；只有用户明确给出正数预算时才传 `token_budget`，零数、负数或模型估算都省略。
