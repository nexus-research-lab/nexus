...
只在调用 `block_work`、`resume_work`、`audit_execution_alignment`、`promote_execution_to_goal`，或处理 Execution/Goal 收口时读取本文件。字段与枚举以当前工具 schema 为准。
...
- exact WorkBinding 可按 schema 供应 locator；unbound round 必须显式定位 Work Item。显式 locator 必须相互一致并匹配当前图。

## audit_execution_alignment

- 只用于 current、非 terminal Execution 的可选 Gate。逐条精确复制当前 completion criteria，按 schema 提交三态 decision、每项 status、证据或具体 gap，以及 aggregate summary。
...
2. 只有 confirmed Goal binding 才继续读取 receipt 中 domain-qualified Goal action；切换到 `goal-manager`，在同一 physical round 通过 `nexus.goal_write` 执行 `audit_objective_alignment`。
3. aligned Goal audit 之后才执行 Goal `update_goal status=complete`。WorkGraph-only 到 Execution terminal 即结束；Goal-only 不走本流程。
4. Goal completion 被拒绝时按返回的 `domain + operation` 恢复：缺 Goal 证据回 Goal audit，未完成责任通过 `nexus.execution_read` 重新读取。不要靠相似 operation 名称猜测恢复路径。
