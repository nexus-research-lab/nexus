...
- 用户只能在完成态 WorkGraph 标题栏请求“保存为草图”。宿主先用 owner 的默认后台模型从完整实际图自动选择、抽象结构，并按界面语言生成有时效的 preview；生成模型会收到当前命令目录与固定保留名，`slash_name` 优先选择不重复的 1 至 3 个短词，服务端仍做最终冲突校验。用户可直接修改元信息，或在宿主从 exact transcript boundary 创建的短期受限 DM 分支中让模型修改文案、节点、父子结构与依赖；宿主校验完整草图的 revision、DAG、key 主路径与 terminal 交付后，只有用户明确应用才替换原 preview。
