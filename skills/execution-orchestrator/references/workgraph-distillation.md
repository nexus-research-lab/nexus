...
- 用户只能在完成态 WorkGraph 标题栏请求“保存为草图”。宿主先用 owner 的默认后台模型从完整实际图自动选择、抽象结构，生成有时效的只读 preview；用户不选择节点或角色。
...
1. 不运行 `execution inspect`，不读取源图，不分析 key/collaboration，也不修改自动生成的命令名或语义。预览已经完成这些工作；CLI 会按 owner、当前 Session、有效期严格校验。
