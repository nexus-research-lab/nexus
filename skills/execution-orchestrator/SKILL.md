...
- `/workgraph` 只启用当前 WorkGraph 协作；只有宿主在用户确认草图后启动 `HiddenFromUser` 的内部 `workgraph_distillation` round，并提供 exact `preview_id` 时才调用 `distill_workgraph`。用户可能已在宿主的受限临时编辑 Session 中修改文案、节点和依赖；该保存 round 只原样保存最终确认预览，不得重新 inspect、选节点、命名、抽象或修订结构，不得要求或制造一条可见聊天消息，预览失效时请用户重新生成，不能绕过。
