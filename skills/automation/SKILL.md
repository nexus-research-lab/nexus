...
操作字段、schedule 形状和创建/修改语义见 [references/operations.md](references/operations.md)。只有在当前动作需要时完整读取该文件。
...
- 外部 IM 的空 list/report 默认只覆盖当前会话；“这里/当前群/当前会话”会按结构化 Session 过滤来源、执行和投递目标。
...
- 提醒、报表、周期自检和持续关注都使用 scheduled task。
