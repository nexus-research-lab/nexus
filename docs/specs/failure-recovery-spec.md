...
| `unknown` | 标题只说明一次“结果待确认”；正文写仍可证明的当前状态和重复操作的具体风险，不枚举“可能成功、也可能失败”等分支 | 先通过领域权威状态对账；普通“重试”不得成为主动作 |
...
- 结果未知只在标题或首个短句中表达一次。正文不展开“可能 A、也可能 B”的分支树，而应写仍然确定的事实、待核对的对象，以及“重复创建风险”“覆盖风险”等具体后果。
...
| 修改请求结果未知 | 任务创建结果待确认 | 当前列表尚未核对；再次创建有重复任务风险 | 先检查任务列表 / **检查任务** |
