...
需要用户在两个互斥业务结果中做选择（例如采用最新版本或覆盖它）时，该区域是独立 decision surface，不是 error recovery。它可以提供两个明确选择，但 error 态类型不得借此恢复第二动作。
...
- 密码修改使用 user-scoped exact `request_id`，终态回执为 `committed|not_applied`。commit 事务必须先领取该 request 的 committed 终态，再对已验证的旧 hash 做 CAS，并将两者一起提交，使两个窗口不能同时用同一旧密码覆盖彼此；`not_applied` 回执与 committed 领取互斥，必须阻止同 request 的迟到写入。浏览器只可持久不含秘密的 request 指针；unknown 时不得创建 fresh request，同一页面仍保有原草稿时可由用户显式续行同一 exact request，草稿已丢失时只能显式放弃并由服务端原子写入 `not_applied` 后解锁。receipt 不存在本身不证明并发请求不会提交。
- Subscription Admin 的未对账 mutation 与可见 feedback 是两个独立状态；dismiss、读取失败或其他提示替换都不得解锁。服务端只有在写入调用前失败时返回 `not_applied`；写入调用报错必须按 `unknown`，写入成功但回读失败才返回 `committed`。
