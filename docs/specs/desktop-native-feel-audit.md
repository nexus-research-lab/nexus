...
- Windows 构建入口新增 `scripts/desktop/build-windows-app.ps1`、`scripts/desktop/smoke-windows-app.ps1`、`scripts/desktop/package-windows-app.ps1`、`make app-win-build`、`make app-win-smoke` 和 `make app-win-package`，默认组装到 `desktop/windows/.build/app/Nexus/`，package 脚本会构建、烟测并输出 Inno Setup 安装器、sha256 和 metadata。
- GitHub `Publish Release` workflow 新增 `windows_app` job，在 `windows-latest` 上执行 Windows app package 脚本，并把 `NexusSetup-*.exe`、安装器 `.sha256` 和 `Nexus-windows-*.metadata.json` 交给最终 `release` job 上传到同一个 GitHub Release；配置 Windows 签名 secret 时会自动做 Authenticode 签名。
