...
Nexus 已有 `skills/imagegen` 和内置 `nexus_imagegen` runtime tool，并且已经做了更符合本平台的改造：

- 默认执行入口是 `nexus_imagegen` MCP 工具。
- `nexusctl imagegen` 保留为显式 CLI / Provider / model 控制的兜底入口。
- Provider、鉴权、接口兼容和响应解析由 Go 服务负责。
- 普通单图走快路径，避免读取 reference、避免反复打开二进制图片。
- 输出以工具 JSON 的 `item.path` / `item.markdown` 为真相源。
...
- 不要让 `imagegen` 回退到纯脚本或前端直连 API。Nexus 已经把 provider 和鉴权放在 Go 服务，这是更适合产品化的边界。
...
4. `skills/imagegen` 增量同步透明图规则：保留内置工具快路径，只在复杂任务读取 reference。
