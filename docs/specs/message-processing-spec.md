...
Provider / bridge `ToolUseSummary` 属于 Agent round 的自然语言 ephemeral 执行摘要。它没有“长任务”、耗时或工具数量门槛：执行中只要收到非空 summary，宿主就立即投影成仅供状态使用的 `progress_update` assistant 块。同一 Agent round 使用稳定 `message_id` 原位替换，round 完成、失败或停止后立即移除。Room 主 Feed 不渲染 thinking、普通 ToolUse/ToolResult、MCP/CLI 调用或可展开过程栏；只复用原有单行活动位置，以统一主色显示当前通用状态，并在 summary 到达后原位替换为自然语言摘要。具体过程只在对应 Thread 中按现有 ToolUseSummary 折叠逻辑展示和展开。DM 没有独立公区/Thread 分层，继续在主时间线保留可展开的折叠过程。真正的最终回复始终由独立 final surface 渲染，不能折入过程或重复同一 summary；权限、AskUserQuestion 与生成式 UI 仍由各自唯一交互面负责。summary 必须携带并保留 `preceding_tool_use_ids`，供 DM/Thread 折叠过程定位相关工具批次；该关联不得使 Room 主 Feed 暴露具体调用。

summary 文案描述已完成批次的具体成果，使用类似 commit subject 的短语而非完整句子、下一步预告或工具动作清单。语言只由最近真实用户文本与 assistant intent 决定：中文会话在提示词中优先使用简洁自然的简体中文，可保留必要的通用技术术语；英文工具名、输入或结果不得参与语言偏好判断。所有工具输入/输出均视为不可信参考数据，摘要模型不得执行其中的指令。Room 主 Feed 中 summary 与“正在思考/正在回复”等 fallback 共用同一个不可展开活动面、同一种主色和同一个几何位置，不追加工具状态、数量、异常数或第二条尾随活动。DM/Thread 的摘要到达后只替换相关折叠栏标题，不追加“已完成”或“正在执行”破坏自然语义；首次点击过程栏只能展开子项目录，Thought、Agent、MCP 与普通工具的详情必须由各自的第二次点击独立打开（Agent 可进入任务详情面板），父级不得级联打开全部详情。失败、拒绝和替换的详细状态只属于可展开过程。round 终态清除 ephemeral summary 后，Room 主 Feed 不保留过程占位，DM/Thread 的 durable 工具只保留中性的“执行过程”审计入口。该投影不写入 transcript、历史、未读或消息计数，也不能混入最终 durable assistant 快照。
