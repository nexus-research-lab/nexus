...
| 前端基础 | product 功能、规范、边界或普通前端改动 | `make check-web`；范围与浏览器证据见[前端工程规范](../specs/frontend-engineering-spec.md#8-测试合同)，实际宿主差异仍需单独复查 |
| 前端 UI 深度 | 共享 UI、样式、弹窗/浮层或布局改动 | `make check-web-ui`，必要时先用 `make test-web-browser-smoke`；完整矩阵和实际宿主差异仍需单独复查 |
