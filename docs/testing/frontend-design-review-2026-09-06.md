...
设置侧栏返回按钮显示完整「返回工作台」，保持与搜索同排；Nexus 改为固定 24px、600 字重、紧凑字距的纯色 sans 字标，删除逐字双层阴影和渐变。仅修改导航呈现，不改变路由。设置导航 3 项 DOM 回归与定向 lint 验证；按用户要求不做视觉验收，清单状态不提升。


### A135 — 保留字标颜色

按用户纠正，恢复 Nexus 字标原有主题渐变、阴影和悬停透明度，仅保留 A134 的高度与紧凑排版调整。定向 lint 通过；不做视觉验收。


### A136 — 名称层级与字体兜底说明

单聊/群聊共同 ConversationRow 移除 inactiveTone=muted，复用公共行默认文字色，保留浅色摘要与现有字号。字体输入只在目录不可用时出现，保留其跨宿主兜底职责；长占位说明移至可换行且由 aria-describedby 关联的下方说明。定向 DOM 回归及 lint 验证，不做视觉验收。


### A137 — 字体入口仅保留下拉

按用户最终要求移除手动字体输入及说明，删除无消费者的四组双语键与 hasRead 状态。原生字体读取、浏览器授权、失败提示及重开重试保留；已保存的自定义字体仍进入下拉，不重置用户偏好。10 项 DOM 回归、定向 lint 和 typecheck 验证；不进行宿主视觉验证，用户环境目录读取原因尚未确认。


### A138 — 模型能力标签可点击

CapabilitySwitch 用原生 label 包含唯一 GlassSwitch，扩大文字点击区域且不复制点击处理；长名称允许换行，避免模型配置两列布局截断。新增 DOM 测试验证文字点击仅触发对应能力一次，另一开关仍可由空格操作。定向测试与 lint 通过；不做视觉验收。
...
补齐 A140 边界：正文、过程与结构化 final 的文件卡片统一抽出到尾部，移除原位置副本，同时按剩余内容重排流式索引。新增展开过程与独立产物场景，8 项文件 DOM 回归验证唯一卡片、正文顺序与原有来源/下载合同；定向 lint/typecheck 通过。Room 公区仍保持既有策略。


### A142 — 搜索收展语义修正

默认完整返回文案加搜索图标，显式点击或键盘激活后挂载并聚焦输入，返回仅留箭头但保留辅助名称。查询非空保持展开，清空且焦点离开整行后收起。消息明细默认 tone 在悬停和展开时均保持 muted，常规字重与异常语义色保留。4 项 DOM 回归、lint/typecheck 验证；不做视觉验收。
...
- `npm run test:contracts`：337 项中 336 通过；唯一失败为 `native-ui-fixtures.test.mjs` 的 localhost listener 被沙箱 EPERM 拒绝。同一夹具以允许监听的权限独立重跑，1 项通过，不启动产品服务或视觉验收。
- `npm run test:components`：290 个文件、1371 项测试全部通过，包含设置搜索、字体选择、主聊天身份与排序、文件产物尾部、代码块合并及原有行为回归。
- A144 已执行 typecheck 与生产构建；构建仍有既有大 chunk 提示。本批不重复无变化的构建。
- 未运行浏览器/原生视觉检查，不以 DOM 或合同结果冒充视觉验收。当前工作区存在其他任务的字标与浏览器用例改动，本批仅记录所测试工作树事实，不合并这些改动。
...
移除配对名称 flex-1，授权状态紧跟名称并与其垂直居中；名称行支持窄宽度换行，避免状态远离所属对象。处理智能体与停用/删除动作不变。配对列表 3 项 DOM 回归和定向 lint 通过；未做视觉验证。


### A149 — Tooltip 空文案关联

共享 Tooltip 以非空文案的实际可见态驱动浮层与 aria-describedby，避免聚焦空提示时引用不存在节点；保留消费者已有说明。新增动态空/非空标签回归，连同 Escape、hover 与嵌套浮层共 10 项测试和 lint 通过；未做视觉验收。


### A150 — 引导卡片语义与换行

TourOverlayCard 增加每实例标题/说明关联的非模态 region，装饰图标不重复朗读；长标题、说明与条目允许连续字符串断行，底部操作可换行。现有公共文字角色与 Button 不变。卡片导航和页面点击退出 2 项回归及 lint 通过；不作像素或视觉验收声明。
...
共享滚动 Hook 对 Ctrl/Meta 与已消费的滚轮事件保持默认行为，避免触控板缩放被转换成标签滚动；保留像素、行、页单位归一和边界放行。删除仅内部调用的滚轮辅助函数导出，补充 L3 与目录合同。新增 3 项原生事件回归，连同标签命令 2 项共 5 项通过，目标 lint 通过；未做视觉验收。


### A154 — 外观数字输入确认边界

AppearanceNumberInput 的 Enter 收口忽略输入法组合态与 229 兼容事件；外部值更新后不再显示与当前值不符的调整提示。保留输入过程中先警告、失焦再收口的行为。新增输入法/普通 Enter、清空恢复和外部更新回归，连同范围测试共 4 项及目标 lint 通过。修正文档中已移除的手工字体输入说明，未做视觉验收。
...
卡片自身的更多/菜单/编辑/删除/运行/历史/详情名称与 mutation 保护提示进入双语目录，保留原有命令和禁用规则。新增语言切换后同一按钮及精确运行目标回归，8 项测试和目标 lint 通过。board model 中状态/计划文案仍有待后续语言审查，本批不声称整个看板双语完成；未做视觉验收。


### A164 — 定时卡片 Agent ID 回退

发现 contextLabel 在来源无有效名称时直接返回执行 agent_id。改为双语中性智能体任务/工作区脚本标签，仍保留匹配来源的可用名称；身份仅用于原来的绑定校验和命令。新增中英无名称和有效名称回归，10 项卡片测试、lint/typecheck 通过。未改状态分列与运行资格，模型其余状态文案仍待审查；未做视觉验收。
...
复核 A154/A156 后发现局部 isComposing/keyCode 组合未复用已有 isImeKeyboardEvent，遗漏 which 兼容。外观数字 Enter 与引导 Escape 改消费唯一浏览器中立工具，补充 which=229 路径，保留默认事件消费逻辑。6 项行为测试及 lint 通过；频道说明链接本轮只读检查，保留原生锚点，不声称完成其视觉审查。


### A170 — MCP 工具目录长文本约束

固定 Connector 与自定义 MCP 共用工具目录补齐标题、工具标识、说明和参数的连续长文本换行，参数限制在容器宽度内；保持服务端原文、共享排版与现有状态逻辑。两个详情消费者共 5 项回归及目标 lint 通过。未做视觉验证，不能据此证明实际窄窗几何；刷新失败保留快照分支仍需后续连同读取所有者检查。


### A171 — MCP 工具读取配置隔离

检查 A170 的刷新失败分支前发现读取 Hook 只拒绝迟到响应，已完成快照未绑定配置身份。固定 Connector 与 Custom MCP 的快照/错误现绑定既有 identity，切换配置时渲染阶段隐藏旧内容；同身份刷新继续保留。新增 4 项双控制器回归覆盖同配置保留、跨配置隐藏、迟到响应和错误切换；目标 lint/typecheck 通过。未改工具视图的失败分支、未做视觉验证；同配置失败提示仍待下一批。


### A172 — MCP 工具刷新就地反馈

同身份普通读取失败保留目录并用公共 UiInlineNotice 双语提示快照可能过期；首次失败继续资源错误态，认证/权限失败隐藏快照。不可用时隐藏重试，读取中禁止重复点击。6 项视图回归加 4 项身份回归、目标 lint/typecheck 通过，未做视觉验收。


### A173 — 定时任务运行与返回目标层级

根据用户对“全部”和房主选项的反馈，按用户最终澄清保留类型筛选并明确为“全部类型 / DM / Room”，保留搜索、对象图标和会话两列。执行/回复 Room 成员选择由高级区移到各自目标下，统一 TaskRoomAgentPicker 并关联字段说明，默认房主与显式失效项语义保留。删除高级区已无调用的 Session/Resource 包装及无用投递 props，权限选择改用 shared plain surface。DM 身份仍由智能体及其会话确定，两方向不互相写入。31 项表单回归、12 项会话/投递契约、目标 lint/typecheck 通过；未做视觉验收。

A173 补充：用户说明旧界面下拉只见“全部”；本轮明确断言三项完整存在，逐项选择 DM、全部、Room 验证对象过滤且不触发执行/投递写入。13 项表单展示回归与 lint 再次通过；没有浏览器视觉验证，未断言旧截图的具体故障根因。


### A174 — 目标类型本地化

按用户要求，目标筛选中文显示“全部类型 / 智能体 / 群聊”，英文为“All types / DM / Room”。仅将标签交给双语目录，不改内部 agent/room 类型或过滤逻辑。13 项表单回归及目标 lint 通过，未做视觉验证。


### A175 — 旧目标投影清理与嵌套菜单遮挡

新版选择器已无生产调用的四个旧目标/会话投影及相关类型/帮助函数删除；失效绑定测试转向当前目标模型，删除无效 isEditing 透传并同步既有 Gallery 消费者，不扩展 Gallery。用户截图进一步显示下拉箭头展开但选项不可见：当前主题 select 120、popover 140 且同 Portal 容器，确认父层盖住子菜单的层级冲突。公共浮层注册表按真实锚点关系协调同容器父子层级，保留语义 token 下限，覆盖逆序注册、多层嵌套及注销恢复。31 项表单测试在清理后通过；最终 22 项层级/关闭焦点/表单测试、目标 lint/typecheck 通过。未做视觉验证，旧 DOM 选项存在性不能证明可见性。


### A176 — 公共嵌套浮层组合回归

补充真实 useAnchoredOverlayLayer + UiSelectMenu + Portal 组合，给父/子配置 140/120 样式，验证同容器子菜单最终层级高于父层，并逐层 Escape 关闭和归还焦点。菜单与浮层目录共 9 文件 107 项测试、目标 lint 全通过。本项验证 DOM/计算样式合同，未声称浏览器绘制或视觉验收完成。


### A177 — 直接凭证弹窗本地化

高德、滴滴、钉钉 AI 表格、腾讯文档、语雀凭证说明与占位及通用标题/文档/连接动作接入双语目录，保持公共 Dialog/Field/Typography。新增中英切换保留同一密码字段、原始草稿及精确 trimmed 提交回归；原测试显式设定中文环境。4 项授权回归、目标 lint/typecheck 通过。未做视觉验收；OAuth 客户端其余中文文案仍待后续审查。


### A178 — 生成文件列表按打开目标去重

用户截图显示一次回复重复出现同名文件卡；现有列表逐条展示独立工具产物记录。新增列表纯模型，以显式 workspace Agent 优先/来源兜底与实际 path（和动作一致 trim）作为目标身份，同目标保留最新记录和首次顺序；不按 basename/display_path/source_tool_use_id 合并，不推断未知归属或路径别名。全部列表消费者复用此边界，原始内容块/历史证据及单独文件卡不改。7 次写入单文件仅展示 1 卡且打开/下载目标精确保留，跨目录/工作区同名及未知来源仍分别保留。10 项相关回归、lint/typecheck 通过，未做视觉验证。
...
移除 Room result 跳过共享产物提取的分支，让本轮 direct/process/final 的结构化文件统一在回复尾部展示并按既有 owner/path 去重；公区仍不渲染具体工具过程。不从 Markdown 路径猜测产物，也不跨轮或跨 Agent 搜集文件。新增群聊过程产物、最终正文中途产物移至末尾、跨投影去重、精确打开/下载归属和无结构化证据不造卡回归。12 项产物测试与 4 项过程测试、目标 lint/typecheck 通过；未做视觉验证。


### A184 — 工具头键盘事件与展开状态

共享工具头原键盘监听会处理内部复制按钮冒泡的 Enter/空格，导致复制时关闭详情且阻止原生按键动作。仅在自身为事件 target 时处理行键盘激活，保留子动作的原生行为；可展开行增加 aria-expanded。真实头部回归覆盖行 Enter/空格、复制 Enter/空格及状态稳定，7 项相关测试、目标 lint/typecheck 通过；审计清单继续在进行中，未做视觉验证。


### A185 — 授权表单实例级字段关联

直接凭证原按 connector ID、OAuth 原使用固定字段 ID，重复挂载时标签会解析到另一表单。使用 useId 生成实例级字段关联，不参与草稿重置身份或提交参数；OAuth Secret name 去除飞书专有命名。新增两个真实重复弹窗回归，验证顶层标签聚焦/键盘输入只作用于自身表单，底层输入保持为空。7 项授权测试、目标 lint/typecheck 通过；未做视觉验证。
