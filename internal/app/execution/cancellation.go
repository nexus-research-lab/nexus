...
// CancellationRoomConsumer 消费 Room slot 的精确取消请求。
type CancellationRoomConsumer interface {
...
// CancellationRuntime 中断指定 runtime Session 的精确 round。
type CancellationRuntime interface {
	InterruptRound(
		context.Context,
		string,
		string,
...
type cancellationConsumer struct {
	room    CancellationRoomConsumer
	runtime CancellationRuntime
}

// NewCancellationConsumer 创建 Orchestration 到 Room/runtime 的取消适配器。
func NewCancellationConsumer(
	room CancellationRoomConsumer,
	runtime CancellationRuntime,
) *cancellationConsumer {
	return &cancellationConsumer{room: room, runtime: runtime}
}

func (c cancellationConsumer) DeliverExecutionCancellation(
	ctx context.Context,
