...
// AgentResolver 返回 runtime 授权判断所需的最新 Agent 记录。
type AgentResolver interface {
	GetAgent(context.Context, string) (*protocol.Agent, error)
}

// mcpSurfaceContext 只根据不会在轮次间漂移的 Session 拓扑选择工具表。
func mcpSurfaceContext(agentID string, sessionKey string) (string, string, bool) {
