...
type stubExecutionGoalService struct {
...
func (s *stubExecutionGoalService) CurrentOptional(
	context.Context,
	string,
) (*protocol.Goal, error) {
	s.currentCalls++
...
func (s *stubExecutionGoalService) Create(
	_ context.Context,
...
func TestExecutionGoalPromotionGatewayRespectsFeatureConfiguration(t *testing.T) {
	tests := []struct {
		name   string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			goals := &stubExecutionGoalService{}
			gateway := newExecutionGoalPromotionGateway(test.config, goals)
			_, err := gateway.PromoteExecution(context.Background(), executionGoalPromotionRequest())
			...
			availability, availabilityErr := gateway.ReadGoalPromotionAvailability(
				...
				orchestrationsvc.GoalPromotionAvailabilityRequest{
					Snapshot: executionGoalPromotionRequest().Snapshot,
				},
			)
...
func TestExecutionGoalPromotionGatewayCreatesFromCanonicalExecution(t *testing.T) {
	goals := &stubExecutionGoalService{
		createResult: &protocol.Goal{
			ID:         "goal-1",
	...
	gateway := newExecutionGoalPromotionGateway(config.Config{
		GoalEnabled:             true,
		GoalAutoContinueEnabled: true,
	}, goals)
	request := executionGoalPromotionRequest()
...
func TestExecutionGoalPromotionGatewayReusesGoalForSameExecution(t *testing.T) {
	goals := &stubExecutionGoalService{current: &protocol.Goal{
		ID:         "goal-existing",
		SessionKey: "room:group:conversation-1",
		Objective:  "Canonical Execution objective",
		Status:     protocol.GoalStatusActive,
	...
	binding, err := gateway.PromoteExecution(context.Background(), executionGoalPromotionRequest())
	if err != nil {
		t.Fatalf("PromoteExecution() error = %v", err)
	}
...
func TestExecutionGoalPromotionGatewayRejectsUnrelatedCurrentGoal(t *testing.T) {
	goals := &stubExecutionGoalService{current: &protocol.Goal{
		ID:         "goal-unrelated",
		SessionKey: "room:group:conversation-1",
		Status:     protocol.GoalStatusActive,
...
func TestExecutionGoalPromotionGatewayRejectsSpoofedOrRetargetedBinding(t *testing.T) {
	tests := []struct {
		name     string
		goal     protocol.Goal
		metadata map[string]any
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			goals := &stubExecutionGoalService{current: &test.goal}
			gateway := newExecutionGoalPromotionGateway(config.Config{
				GoalEnabled:             true,
				GoalAutoContinueEnabled: true,
			}, goals)
...
func TestExecutionGoalPromotionGatewayMapsConcurrentCreateConflict(t *testing.T) {
	goals := &stubExecutionGoalService{createErr: goalsvc.ErrGoalConflict}
	gateway := newExecutionGoalPromotionGateway(config.Config{
		GoalEnabled:             true,
		GoalAutoContinueEnabled: true,
	}, goals)
...
func executionGoalPromotionRequest() orchestrationsvc.GoalPromotionRequest {
