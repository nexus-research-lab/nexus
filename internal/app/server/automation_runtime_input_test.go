...
func TestPrepareAutomationRuntimeInputIsPrivateStableAndRoundScoped(t *testing.T) {
	t.Setenv(appfs.NexusStateRootEnvName, t.TempDir())
	firstPath, cleanup, err := prepareAutomationRuntimeInput("owner-a", "round-a", "capability-a")
	if err != nil {
		t.Fatalf("prepareAutomationRuntimeInput() error = %v", err)
	}
	defer cleanup()
	...
	reusedPath, reusedCleanup, err := prepareAutomationRuntimeInput("owner-a", "round-a", "capability-a")
	if err != nil {
		t.Fatal(err)
	}
	defer reusedCleanup()
	...
	secondPath, secondCleanup, err := prepareAutomationRuntimeInput("owner-a", "round-b", "capability-a")
	if err != nil {
		t.Fatal(err)
	}
	defer secondCleanup()
	...
	staleAt := time.Now().Add(-automationRuntimeInputRetention - time.Hour)
	if err = os.Chtimes(staleDirectory, staleAt, staleAt); err != nil {
		t.Fatal(err)
	}
	_, thirdCleanup, err := prepareAutomationRuntimeInput("owner-a", "round-c", "capability-a")
	if err != nil {
		t.Fatal(err)
	}
	defer thirdCleanup()
