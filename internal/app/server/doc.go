...
//   - routes.go / routes_web.go / http_handlers.go / websocket.go：HTTP/Web 路由、含 Computer Use settings lifecycle 的 HTTP handlerSet 装配、WS 入口与 orchestration ExecutionInvalidationSink 装配。
//   - *_mcp.go / channel_authorization.go / connector_authorization.go / runtime_mcp_authority.go：authorization / communication / connector / visualize / imagegen / browser / room 内建 MCP server 按 Session 拓扑稳定装配；Channel/Connector 真人授权 builder 与共享可信身份判定在 *_authorization.go 与 runtime_mcp_authority.go；Goal/Execution/Automation 不再挂载 MCP。
//   - configuration_runtime.go / runtime_command.go / runtime_command_input.go：nexuscfg loopback broker 与 Agent-facing nexus CLI 的 Goal/Execution/Automation/Computer Use command broker、命名工作图保存 operation、command capability 环境、round 私有 0600 JSON 输入槽、调用时读取的动态 SDK Session identity 与 typed mutation receipt；不直接向 runtime 开放数据库、CUA token 或 endpoint。
