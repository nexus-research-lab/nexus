...
//   - *_mcp.go：authorization / communication / connector / execution / goal / visualize / imagegen / room 内建 MCP server 按 Session 拓扑稳定装配；DM runner / Room slot 逐轮绑定统一动态 Responsibility authority、exact Goal/revision/Execution、trusted WorkBinding/ReviewBinding 与 human authority。
//   - configuration_runtime.go / automation_runtime.go / automation_runtime_input.go：nexuscfg 与 Agent-facing nexus CLI 的 loopback broker、physical-round capability、round 私有 JSON 输入槽、领域确认与 service 路由；不直接向 runtime 开放数据库。
