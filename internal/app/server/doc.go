...
//
// 成员清单：
//   - server.go / lifecycle.go / app_services.go / core_services.go：HTTP 进程生命周期、可选 Team Relay client、本地投影与总依赖装配；跨进程恢复器和后台 worker 仍由组合根启动。
...
//   - routes.go / routes_web.go / path_param_router.go / http_handlers.go / websocket.go：HTTP/Web/可选 Team 路由、统一路径段解码（含 Provider 历史 model_id 兼容边界）、HTTP handlerSet 装配、WS 入口与 orchestration ExecutionInvalidationSink 装配。
