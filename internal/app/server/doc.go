...
//   - configuration_runtime.go / runtime_command.go / runtime_command_input.go：nexuscfg loopback broker 与 Agent-facing nexus CLI 的 Goal/Execution/Automation command broker、命名工作图保存 operation、command capability 环境、round 私有 0600 JSON 输入槽、调用时读取的动态 SDK Session identity 与 typed mutation receipt；不直接向 runtime 开放数据库。
