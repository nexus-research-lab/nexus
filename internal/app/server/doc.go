...
//   - configuration_runtime.go / runtime_command.go：nexuscfg loopback broker，以及 Goal/Execution/Automation 共用的 round-scoped `nexus_runtime.command`、命名工作图保存 operation、动态 SDK Session identity 与 typed mutation receipt；结构化业务输入经现有 SDK MCP 通道直达领域服务，不落临时 JSON，也不向 runtime 开放数据库。
