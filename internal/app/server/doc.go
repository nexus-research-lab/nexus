...
//   - *_mcp.go：authorization / communication / connector / visualize / imagegen / room 内建 MCP server 按 Session 拓扑稳定装配；Goal/Execution/Automation 不再挂载 MCP。
//   - configuration_runtime.go / automation_runtime.go / automation_runtime_input.go：nexuscfg 与 Agent-facing nexus CLI 的 loopback broker、physical-round capability、round 私有 JSON 输入槽、调用时读取的动态 SDK Session identity、Goal/Execution portable schema 前置校验、领域确认与 service 路由；不直接向 runtime 开放数据库。
