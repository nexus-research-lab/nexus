...
func NewAppServicesWithDB(cfg config.Config, db *sql.DB, logger *slog.Logger) *AppServices {
	...
	channelControl.SetRoomService(core.Room)
	...
	dmService.SetReplyPreviewRepository(newRoomRepository(cfg, db))
	...
	ingressService.SetRoundIndexReader(core.Session.GetSessionRoundIndex)
	...
	ingressService.SetRoomRealtime(roomRealtime)
	roomRealtime.SetReplyPreviewRepository(newRoomRepository(cfg, db))
