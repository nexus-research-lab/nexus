...
func structuredResult(_ string, content map[string]any) command.Result {
	text := "{}"
...
func appliedResult(summary string, content map[string]any) command.Result {
...
func errorResult(err error) command.Result {
...
func errorResultText(text string) command.Result {
	return command.Result{
		Content: []map[string]any{{
			"type": "text",
			"text": text,
...
func errorResultWithNextAction(err error, nextAction map[string]any) command.Result {
	text := "goal command failed"
	if err != nil {
		text = err.Error()
	}
...
func planModeGoalMutationResult(operationName string) command.Result {
