...
func trustedWorkReferenceProperties() map[string]any {
	...
	properties["work_item_id"] = stringProperty("Conditional Work Item locator. Only an exact trusted WorkBinding permits omission and supplies its Work Item; assigned_work/current_actor projections do not establish that binding. An explicit value must match. In an unbound DM call, provide work_item_id or logical_key. In an unbound Room conversational round, explicit locators do not grant mutation authority; the verified coordinator must call get_execution first, while another Room actor requires an exact trusted WorkBinding.")
	properties["logical_key"] = stringProperty("Conditional stable Work Item locator. Only an exact trusted WorkBinding permits omission and supplies its Work Item; assigned_work/current_actor projections do not establish that binding. An explicit value must match. In an unbound DM call, provide logical_key when work_item_id is absent. In an unbound Room conversational round, explicit locators do not grant mutation authority; the verified coordinator must call get_execution first, while another Room actor requires an exact trusted WorkBinding.")
	return properties
}
...
func submitWorkSchema() map[string]any {
	...
	properties["assignment_id"] = stringProperty("Optional current Assignment id; it never replaces the required Work Item locator in an unbound DM call or grants Room mutation authority. Omit inside an exact trusted WorkBinding to use its Assignment. Otherwise the backend selects the current Assignment for the explicit Work Item; any explicit value must match.")
...
func reviewWorkSchema() map[string]any {
	properties := workReferenceProperties()
	properties["work_item_id"] = stringProperty("Conditional review locator. Only an exact trusted ReviewBinding, or a permitted self-review exact trusted WorkBinding, permits omission and supplies its Work Item; assigned_work/current_actor projections do not establish either binding. An explicit value must match. In an unbound DM call, provide at least one of submission_id, work_item_id, or logical_key, and all supplied locators must identify the same Work Item. In an unbound Room conversational round, explicit locators do not grant mutation authority; the verified coordinator must call get_execution first, while another Room actor requires an exact trusted ReviewBinding or permitted self-review WorkBinding.")
	properties["logical_key"] = stringProperty("Conditional stable review locator. Only an exact trusted ReviewBinding, or a permitted self-review exact trusted WorkBinding, permits omission and supplies its Work Item; assigned_work/current_actor projections do not establish either binding. An explicit value must match. In an unbound DM call, provide at least one of submission_id, work_item_id, or logical_key, and all supplied locators must identify the same Work Item. In an unbound Room conversational round, explicit locators do not grant mutation authority; the verified coordinator must call get_execution first, while another Room actor requires an exact trusted ReviewBinding or permitted self-review WorkBinding.")
	properties["submission_id"] = stringProperty("Conditional immutable Submission locator. An exact trusted ReviewBinding permits omission and supplies its Submission; a permitted self-review exact trusted WorkBinding selects the current unreviewed Submission for its Work Item. assigned_work/current_actor projections do not establish either binding. Explicit values must match the bound target. In an unbound DM call, provide at least one of submission_id, work_item_id, or logical_key; a Work Item locator selects its current unreviewed Submission. In an unbound Room conversational round, explicit locators do not grant mutation authority; the verified coordinator must call get_execution first, while another Room actor requires an exact trusted ReviewBinding or permitted self-review WorkBinding.")
