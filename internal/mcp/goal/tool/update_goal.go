	...
	return sdktool.Tool{
		...
		Handler: func(ctx context.Context, input map[string]any) (sdktool.ToolResult, error) {
			...
			if status == protocol.GoalStatusComplete {
				return structuredResult("goal marked complete", goalCompletionMutationPayload(item)), nil
			}
			return structuredResult("goal marked blocked", goalMutationPayload(item)), nil
		},
	}
}
