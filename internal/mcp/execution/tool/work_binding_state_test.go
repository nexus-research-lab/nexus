...
// POS: MCP execution tool tests for host-owned per-assignment round identity
...
func TestAssignWorkBindsOnlyConfirmedRoomSelfReceipt(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			sctx := contract.ServerContext{ScopeKind: test.scope}
			if test.state {
...
func TestReviewWorkClearReceiptReturnsRoomRoundToCoordination(t *testing.T) {
	...
	if !applyMutationWorkBindingTransition(contract.ServerContext{WorkBindingState: state}, result) {
