...
func TestSourceSessionLabelFallback(t *testing.T) {
	source := Source(contract.ServerContext{CurrentSessionKey: "agent:agent-1:dm:dm-user:main:"}, "agent-1")
	if source.SessionLabel != "当前对话" {
		t.Fatalf("session label = %q, want 当前对话", source.SessionLabel)
	}

	source = Source(contract.ServerContext{
		CurrentSessionKey:   "agent:agent-1:dm:dm-user:main:",
		CurrentSessionLabel: "  自定义对话  ",
	}, "agent-1")
	if source.SessionLabel != "自定义对话" {
		t.Fatalf("explicit session label = %q, want 自定义对话", source.SessionLabel)
	}
}
