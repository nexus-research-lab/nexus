...
func report(svc contract.Service, sctx contract.ServerContext) sdktool.Tool {
	return sdktool.Tool{
		Name:        "get_scheduled_task_report",
		Description: "按日期聚合定时任务运行和投递状态。默认查询今天；各 Agent 默认只能查看自己名下任务，owner main 仅在自己的可信私有 DM 可传其他 agent_id 或对应 job_id。当前会话是飞书/IM 群且未指定 job_id/agent_id/query 时，默认聚合当前群相关任务；DM/Room/IM 群里的 query 会优先匹配当前会话任务，显式写“这里/当前会话/这个群/当前频道”会强制限定到当前会话；泛化的“当前会话/这个群定时任务发送情况”会聚合当前会话任务，带具体任务名时仍定位唯一任务。任务明细会返回 signals、suggested_tools、latest_execution_error、latest_delivery_error、recovery_run_id、execution_failed_run_ids、delivery_pending_run_ids、delivery_skipped_run_ids、manual_redelivery_run_ids 和 delivery_dead_letter_run_ids，便于直接解释发送情况、修正执行失败、恢复卡住 run 或补投递失败结果。",
		SearchHint:  searchHintTaskReport,
