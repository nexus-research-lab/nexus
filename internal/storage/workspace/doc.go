...
//     turn projector / rewrite_tail / fork marker 前缀 / 按 Agent 执行轮配对的 result_summary / 同 message_id 合并 Goal 完成收据 / external_delivery / 不为完成态 host control 伪造中断 assistant 的 unfinished_round）。
...
//   - history_read_model.go：宿主 SQLite/B-Tree 派生读模型，以原子 generation、source/payload 校验、有界 metadata window、淘汰与整库版本重建提供按页读取；canonical 历史不迁移。
...
//     保留 Agent 执行身份、public handoff reply 因果且按 ledger 文件版本缓存可见消息计数的房间历史、可反向修复两阶段写入且按稳定 message_id 幂等的 Goal-attributed 定向消息/Room handoff（含 terminal/Goal handback 独立阶段和审计约束的 legacy attribution repair）、immediate/delayed schedule-complete 唤醒、带最后一次上下文占用快照的会话文件、只读目录证据探测与 JSONL。
