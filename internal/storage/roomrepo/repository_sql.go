...
WHERE id = `+r.dialect.Bind(1)+` AND owner_user_id = `+r.dialect.Bind(2), roomID, ownerUserID); err != nil {
			return nil, err
		}
	}

	if err = r.deleteRoomAgentSessionDependents(ctx, tx, roomID, agentID); err != nil {
		return nil, err
	}
...
func (r *SQLRepository) DeleteConversation(ctx context.Context, ownerUserID string, roomID string, conversationID string) (*protocol.ConversationContextAggregate, error) {
	...
	if err = r.deleteConversationDependents(ctx, tx, conversationID); err != nil {
		return nil, err
	}
	result, err := tx.ExecContext(ctx, `DELETE FROM conversations WHERE id = `+r.dialect.Bind(1)+` AND room_id = `+r.dialect.Bind(2), conversationID, roomID)
	if err != nil {
		return nil, err
	}
	affected, err := result.RowsAffected()
