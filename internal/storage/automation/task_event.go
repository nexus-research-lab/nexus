...
func (r *Repository) ListTaskEventsByJob(ctx context.Context, ownerUserID string, jobID string, limit int) ([]automationdomain.ScheduledTaskEvent, error) {
...
WHERE job_id = ` + r.bind(1)
	...
	items := make([]automationdomain.ScheduledTaskEvent, 0)
	for rows.Next() {
...
func (r *Repository) SearchTaskEvents(ctx context.Context, ownerUserID string, agentID string, queryText string, limit int) ([]automationdomain.ScheduledTaskEvent, error) {
...
func scanTaskEvent(scanner interface {
	...
	var item automationdomain.ScheduledTaskEvent
	...
	if err := scanner.Scan(
		...
		return automationdomain.ScheduledTaskEvent{}, err
	}
