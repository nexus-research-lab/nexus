...
WHERE agent_id = ` + r.dialect.Bind(1) + `
   OR job_id IN (SELECT job_id FROM automation_cron_jobs WHERE agent_id = ` + r.dialect.Bind(2) + `)`, args: []any{agentID, agentID}},
...
WHERE job_id IN (SELECT job_id FROM automation_cron_jobs WHERE agent_id = ` + r.dialect.Bind(1) + `)`, args: []any{agentID}},
		{query: `UPDATE automation_cron_jobs SET source_creator_agent_id = NULL WHERE source_creator_agent_id = ` + r.dialect.Bind(1), args: []any{agentID}},
		{query: `DELETE FROM automation_cron_jobs WHERE agent_id = ` + r.dialect.Bind(1), args: []any{agentID}},
