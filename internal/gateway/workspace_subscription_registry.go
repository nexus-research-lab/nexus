...
type workspaceSubscriptionRegistry struct {
	...
	agentSenders    map[string]map[string]*websocketSender
...
func newWorkspaceSubscriptionRegistry(
	...
	return &workspaceSubscriptionRegistry{
		...
		agentSenders:    make(map[string]map[string]*websocketSender),
		lastSnapshots:   make(map[string]runtimeSnapshot),
	}
}

func (r *workspaceSubscriptionRegistry) Subscribe(ctx context.Context, sender *websocketSender, agentID string) error {
	...
	if r.agentSenders[agentID] == nil {
		r.agentSenders[agentID] = make(map[string]*websocketSender)
	}
...
func (r *workspaceSubscriptionRegistry) Unsubscribe(sender *websocketSender, agentID string) {
	if r == nil || sender == nil {
		return
	}
...
func (r *workspaceSubscriptionRegistry) UnregisterSender(sender *websocketSender) {
	if r == nil || sender == nil {
		return
	}
	r.mu.Lock()
...
func (r *workspaceSubscriptionRegistry) broadcastRuntimeChanges() {
	...
	for agentID, senders := range r.agentSenders {
		...
		targets := make([]*websocketSender, 0, len(senders))
...
func (r *workspaceSubscriptionRegistry) sendRuntimeSnapshot(sender *websocketSender, agentID string) {
