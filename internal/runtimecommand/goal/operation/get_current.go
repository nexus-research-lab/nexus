...
func getGoal(svc contract.Service, sctx contract.Context) runtimecommand.Operation {
	return readGoalOperation("get_goal", "Get the current goal for this thread, including status, budgets, token and elapsed-time usage, and remaining token budget.", svc, sctx)
}
