...
func assignWork(svc contract.Service, sctx contract.Context) runtimecommand.Operation {
	const operationName = "assign_work"
...
func submitWork(svc contract.Service, sctx contract.Context) runtimecommand.Operation {
	const operationName = "submit_work"
	return runtimecommand.Operation{
		Name: operationName,
		...
		ContextHandler: func(ctx context.Context, input map[string]any, callContext *runtimecommand.CallContext) (runtimecommand.Result, error) {
			...
			if callContext != nil {
				...
				toolUseID = callContext.RequestID
			}
...
func reviewWork(svc contract.Service, sctx contract.Context) runtimecommand.Operation {
	const operationName = "review_work"
	return runtimecommand.Operation{
		Name: operationName,
...
func blockWork(svc contract.Service, sctx contract.Context) runtimecommand.Operation {
	const operationName = "block_work"
	return runtimecommand.Operation{
		Name:        operationName,
...
func resumeWork(svc contract.Service, sctx contract.Context) runtimecommand.Operation {
	const operationName = "resume_work"
	return runtimecommand.Operation{
		Name:        operationName,
...
func takeOverWork(svc contract.Service, sctx contract.Context) runtimecommand.Operation {
	const operationName = "take_over_work"
	return runtimecommand.Operation{
		Name: operationName,
...
func mutationEnvelope(
...
) (*orchestrationSnapshot, string, *runtimecommand.Result) {
	...
	command, err := commandID(sctx, callContext, operationName, input, snapshot.Execution.Version)
	if err != nil {
		result := transportErrorResult(err)
		return nil, "", &result
	}
