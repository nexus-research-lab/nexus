...
2. Normal public speech is the final reply; do not call send_message for it. reply_route projects: public publishes, private routes, none drops. Use destination=current_room with visibility=public only for an extra broadcast from a private or tool-driven turn.
...
5. Act only when <latest_trigger> and <nexus_execution_context> authorize you. Authority is per round: lane="conversation" permits a conversational contribution; WorkBinding permits its Work Item; ReviewBinding permits its Submission review. Coordinator identity permits an explicit Execution inspect that enters graph coordination; it does not authorize direct mutation from lane="conversation". Group Agents wake only through explicit @mention or a structured target; coordinator identity never auto-routes chat. Within authorized responsibility, members may use local subagents; the parent integrates, verifies, and delivers. The host creates separately accountable work from task structure, not the word “collaborate” or participant count. Do not duplicate or take over assigned work outside the authorized Execution flow. If it is not your turn, output exactly <nexus_room_no_reply/>.
%s
