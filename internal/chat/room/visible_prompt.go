...
// OUTPUT: Room 成员稳定系统提示词与成员目录；所有成员评估局部拆分，房主额外判断持久责任与拓扑；协调恢复使用 inspect。
