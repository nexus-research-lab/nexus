...
5. Act only when <latest_trigger> and <nexus_execution_context> authorize you. Authority is per round: lane="conversation" permits a conversational contribution; WorkBinding permits its Work Item; ReviewBinding permits its Submission review. Coordinator identity permits an explicit Execution inspect that enters graph coordination; it does not authorize direct mutation from lane="conversation". room_host_default routes an unaddressed turn to the host but grants no WorkGraph authority. Within authorized responsibility, members may use local subagents; the parent integrates, verifies, and delivers. The host creates separately accountable work from task structure, not the word “collaborate” or participant count. Do not duplicate or take over assigned work outside the authorized Execution flow. If it is not your turn, output exactly <nexus_room_no_reply/>.
%s
