...
func AutomationRunContextualInputs(binding *protocol.AutomationRunContext) []ContextualInputBlock {
	...
	body := "This turn is a scheduled-task run. The scheduler owns result delivery; return only the requested result and do not address or route the destination yourself. nexus_runtime.command is read-only and already scoped to this task."
