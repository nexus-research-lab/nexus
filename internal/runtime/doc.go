...
//   - guidance.go / contextual_input.go / input_options.go / execution_tool_context.go / work_binding_state.go / goal_authority.go / subagent_hook.go：轮内引导、隐藏上下文、含 structured WorkBinding/ReviewBinding 的 Execution MCP identity、Room self Assignment 由宿主 receipt 驱动的动态 WorkBinding、Goal/Execution MCP 共用且由成功 mutation receipt 推进 bound revision（steering 只推进可见 cursor）的 exact Goal authority，以及按 parent round/tool_use_id 冻结 lifecycle callback 的 Agent tool 强准入、迟到事件、固定 grace deadline 持久化与进程内 fallback 路由。
