...
func TestWorkspacePolicyHookChecksBashAndNexusctlWithoutBlockingSystemTools(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "runtime path displayed as text", command: `echo "NEXUS_COMMAND_PATH=$NEXUS_COMMAND_PATH"`, denied: false},
		{name: "runtime path text before direct call", command: `echo "NEXUS_COMMAND_PATH=$NEXUS_COMMAND_PATH"; "${NEXUS_COMMAND_PATH}" --json execution inspect`, denied: false},
		{name: "powershell runtime path displayed as text", command: `Write-Output "${env:NEXUS_COMMAND_PATH}=present"`, denied: false},
		...
		{name: "forged quoted env command capability", command: `env 'NEXUS_COMMAND_CAPABILITY_TOKEN=fake' nexus automation contract`, denied: true},
		{name: "forged powershell command capability", command: `$env:NEXUS_COMMAND_CAPABILITY_TOKEN = 'fake'; nexus automation contract`, denied: true},
		{name: "forged braced powershell command path", command: `${env:NEXUS_COMMAND_PATH} = 'fake'; nexus execution inspect`, denied: true},
		{name: "forged automation input path", command: `NEXUS_COMMAND_INPUT_PATH=/tmp/fake "${NEXUS_COMMAND_PATH}" --json automation plan --operation update`, denied: true},
