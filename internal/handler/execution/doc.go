...
//
// 成员清单：
//   - handlers.go：按 owner/session 返回当前、exact 及历史 managed ExecutionView、从 exact 完成图生成/复用 Draft、创建或恢复隐藏编辑会话、选择版本、调度不进入聊天时间线的内部保存 round，并列出或删除已保存工作图；命名图持久化只经 Skill + CLI。
//
