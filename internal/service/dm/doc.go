...
//   - history.go / fork.go / transient_fork.go / rewrite.go / title.go / recovery_context.go / transport_context.go / execution_context.go / execution_evidence.go / context_usage.go：历史、同 Agent 延迟物化的 conversation fork、从最近完成助手轮次创建目录隐藏的短期 DM fork、provider-specific 边界位于 transcript 尾部时的完整 source 兼容 fork、先提交 Room SQL 再投影 workspace 的 SDK session/fingerprint 同步、区分 transcript rewrite 与启动失败 overlay-only rerun 的重写、标题、上一轮失败恢复、active-paired IM 的无标识只读 transport 上下文、每轮权威 WorkGraph 上下文与 compact 持久证据，以及每轮终态上下文占用快照持久化。
