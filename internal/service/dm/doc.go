...
//   - history.go / fork.go / transient_fork.go / transient_session.go / rewrite.go / title.go / recovery_context.go / transport_context.go / execution_context.go / execution_evidence.go / context_usage.go：历史、同 Agent 延迟物化的 conversation fork、仍需继承上下文的嵌入场景所用且跳过 overlay-only/局部投影的隐藏 fork、不继承源 transcript 且供 WorkGraph 编辑/保存使用的目录隐藏专用 Session、provider-specific 边界兼容、先提交 Room SQL 再投影 workspace 的 SDK session/fingerprint 同步、区分 transcript rewrite 与启动失败 overlay-only rerun 的重写、标题、上一轮失败恢复、active-paired IM 的无标识只读 transport 上下文、每轮权威 WorkGraph 上下文与 compact 持久证据，以及每轮终态上下文占用快照持久化。
...
//     外部私聊复用同 Agent Skill，Computer Use 按 owner 桌面开关选择 Skill 且 broker
//     再次鉴权，provider init/fork 后的 SDK Session identity 动态写回同轮 command context，受控 Automation 执行可覆盖创建时工具快照且 CLI 只读绑定当前 job/run。
//
