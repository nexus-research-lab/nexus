...
//   - history.go / fork.go / rewrite.go / title.go / recovery_context.go / transport_context.go / execution_context.go / execution_evidence.go / context_usage.go：历史、同 Agent 延迟物化的 conversation fork、先提交 Room SQL 再投影 workspace 的 SDK session/fingerprint 同步、重写、标题、上一轮失败恢复、active-paired IM 的无标识只读 transport 上下文、每轮权威 WorkGraph 上下文与 compact 持久证据，以及每轮终态上下文占用快照持久化。
...
//     工具面变化时从旧 transcript 幂等 fork 新物理 Session，并签发 nexuscfg 与 Agent-facing nexus command 的 physical-round capability；active-paired
