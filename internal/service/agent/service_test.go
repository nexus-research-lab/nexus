...
func TestServiceBootstrapsMainAgentAndCreatesAgent(t *testing.T) {
	...
	if loaded.SkillsCount != 7 {
		t.Fatalf("skills_count 不正确: got=%d want=7", loaded.SkillsCount)
	}
	...
	for _, item := range items {
		if item.AgentID == created.AgentID && item.SkillsCount != 7 {
			t.Fatalf("list_agents skills_count 不正确: got=%d want=7", item.SkillsCount)
		}
	}
