...
// OUTPUT: 含默认产品说明的共享系统 Skill 目录、列表/详情模型和绑定 runtime_version 的 AgentSkillState。
// POS: Skills 服务跨目录、HTTP、配置控制面与 Agent 托管语义 Skill 绑定的协议模型。
package skills
...
func buildSystemSkillNames() map[string]struct{} {
	...
	for _, skillName := range agentsvc.ManagedSemanticSkillNames() {
