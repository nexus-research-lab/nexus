...
//
// 成员清单：
//   - service.go / query.go / history.go / external_identity.go：Service、持久化端口、查询、历史消息，以及
//     供目录首屏、Automation 校验和首次投递物化共用的统一 Session 解析、按字段所有权合并 Room SQL/workspace 纯读投影、不触发历史读取的独立 Room 摘要及正常历史页顺带补齐、可取消的历史分页、当前 generation 大内容按需 detail、按当前 Goal 聚合真相刷新完成收据、
