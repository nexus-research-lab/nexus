...
//     runtime_usage_observation.go 统一子任务证据的单调合并与落库确认；runtimeusage/ 将 DM/Room 消息转换为同一快照，不把宿主锁或 runtime 依赖引入 Goal 主包。
//   - objective.go / alignment.go / preview.go / appserver.go：目标改写、共享 Objective Alignment 报告的 Goal 生命周期适配、以目标正文为回退的首个 Goal 会话标题投影与重启恢复、Codex app-server 语义。
