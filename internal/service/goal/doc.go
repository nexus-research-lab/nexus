...
//   - continuation.go / progress.go / resume.go / steering.go：普通续跑与 awaiting_plan 专用 successor-planning continuation 的 durable reserve/leased-claim/leased-start/terminal-settle/retry/release、root receipt 与 Agent audit 双身份终态、revision 安全的进展与 Room collaboration handback 记录、跨进程恢复、DM/Room runtime 导向与完成后的 result-first 最终交付提示。
...
//   - read.go / errors.go / repository.go / observability.go：exact owner-scoped durable collaboration 读取、跨调用方统一错误分类、持久化契约与后台恢复日志边界。
//
