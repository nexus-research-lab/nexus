...
//   - objective_transition.go / execution_binding.go / execution_binding_read.go / execution_completion.go / clear.go / room_metadata.go / room_command_update.go / room_collaboration.go / room_completion.go / tool.go / retarget.go / event.go / cleanup.go / helpers.go：durable Goal objective revision / Execution rebase saga、含旧显式 Goal reservation fence 与 server-owned reserved/pending/confirmed phase 的反向 Execution binding、严格 owner-scoped 且不持久化的中央 binding 读面、WorkGraph completion 与 clear lifecycle gate、Room creator/lead 权限、host Goal replace 的 server-only Room metadata 合并、revision 安全的协作证据、模型工具状态更新与同 ID 目标替换、事件广播、关联 Goal 探测与清理、辅助。
