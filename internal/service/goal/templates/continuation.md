...
- Load the `goal-manager` Skill and use only `nexus.goal_read` / `nexus.goal_write`; their current schemas are the contract, and request identity is host-owned. Never use nexusctl or inferred standalone operation tools. For a Goal with a confirmed managed WorkGraph binding, before marking it complete invoke `audit_objective_alignment` with one scalar `report_json`. Goal-only completion does not require this audit.
