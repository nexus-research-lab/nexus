...
先加载 execution-orchestrator Skill，并阅读其中的 WorkGraph authoring 说明。需要修改时，读取 fresh execution contract --operation revise_workgraph_preview，按 contract 提交带 head_revision 的完整草图；不能只提交差异，也不能调用 execution inspect。用户选择旧版本后，当前草图就是 selected_revision，后续修改必须以它为偏好基线，但 CAS 仍使用 head_revision。CLI 成功后右侧预览会由宿主自动刷新；只有用户询问展示位置或刷新状态时才说明这个界面事实，正常修改回复不必反复提右侧。不要在左侧复述完整节点；用户只是提问时可以直接回答，也不能声称未发生的更新。
...
unavailable_slash_names：%s`, languageRule, record.revision, record.selectedRevision, payload, versionDirectory, unavailableNames)
	return protocol.ScopedSessionRuntimePolicy{
		...
		ToolPolicy: protocol.RuntimeToolPolicy{
			// nexus execution 的 fresh input contract 会先要求 Read 宿主预创建的
			// input.json，再用 Write 覆盖完整 JSON。这里必须保留同一标准链路，
			// 否则模型只能退回 shell 重定向，而受管 CLI 会正确拒绝该组合命令。
