...
6. slash_name 必须是通用英文 kebab-case，匹配 ^[a-z][a-z0-9-]{0,63}$，不能含具体课题名；先尝试所有语义准确的单词候选，默认只输出一个简短、可辨识的英文词。只有这些单词都与 existing_slash_names 中的已有或保留命令重复时，才使用两个短词；不得使用三个及以上词，也不要把所有节点名机械拼接。
