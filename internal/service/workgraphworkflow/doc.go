...
//   - service.go / save_dispatch.go：完成图抽取/复用、coordinator 绑定的隔离内部保存调度、受管 CLI 幂等保存、读取/删除、Slash descriptor 投影与 runtime prompt 展开；exact editor revision 冲突以独立领域错误在写入前关闭。
...
//
// 内置模板只读且拥有稳定 Slash；历史同名 owner 保存图在升级后优先，避免命令语义被替换。Draft 按 exact source Execution 去重并保存不可变版本、head CAS 与 selected preference；关闭编辑 UI 不删除隐藏会话。修改必须通过 DAG、父子结构、key 主路径与 terminal 交付校验。UI 保存只在不继承源 transcript 的隐藏内部 DM 中把 exact preview_id 交给 Skill + CLI；普通对话则由同一 Skill/CLI 在用户明确确认后保存。运行、工具与交付历史始终留在源 Execution。
