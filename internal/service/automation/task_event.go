...
func (s *Service) ListTaskEvents(ctx context.Context, jobID string, limit int) ([]automationdomain.ScheduledTaskEvent, error) {
	...
	job, err := s.repository.GetScheduledTask(ctx, ownerUserID, jobID)
	if err != nil {
		return nil, err
	}
...
func updateTaskEventAction(input automationdomain.UpdateJobInput, next automationdomain.ScheduledTask) string {
...
func updateTaskEventDetail(input automationdomain.UpdateJobInput, before automationdomain.ScheduledTask, after automationdomain.ScheduledTask) map[string]any {
...
func updateTaskEventRunID(input automationdomain.UpdateJobInput, before automationdomain.ScheduledTask) string {
...
func deleteTaskEventDetail(job automationdomain.ScheduledTask, cancelledRunID string, cancelledRun bool, deadLetteredDeliveryRunIDs []string) map[string]any {
...
func deliveryRetryTaskEventDetail(run automationdomain.ScheduledTaskRun) map[string]any {
	detail := map[string]any{
