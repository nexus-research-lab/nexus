...
func (s *Service) describeScheduledTasksSection(ctx context.Context, agentID string) string {
	...
	if len(jobs) > limit {
		lines = append(lines, fmt.Sprintf("(+%d more, use nexus.automation_read list for full list)", len(jobs)-limit))
	}
	return "Scheduled tasks (manage these through the built-in automation Skill and Nexus Automation tools):\n- " + strings.Join(lines, "\n- ")
}
