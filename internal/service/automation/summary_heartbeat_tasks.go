...
func (s *Service) describeScheduledTasksSection(ctx context.Context, agentID string) string {
	...
	if len(jobs) > limit {
		lines = append(lines, fmt.Sprintf("(+%d more, use nexus automation inspect --operation list for full list)", len(jobs)-limit))
	}
	return "Scheduled tasks (manage these through the built-in automation Skill and nexus automation CLI):\n- " + strings.Join(lines, "\n- ")
}
