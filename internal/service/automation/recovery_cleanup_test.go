...
func TestDisableTaskPreservesActiveRunForRecovery(t *testing.T) {
	...
	if err = db.QueryRow(`SELECT running_run_id, next_run_at FROM automation_scheduled_tasks WHERE job_id = ?`, task.JobID).Scan(&runningRunID, &nextRunAt); err != nil {
	...
	var disableEvent *automationdomain.ScheduledTaskEvent
...
func TestDeleteTaskCancelsActiveRun(t *testing.T) {
	...
	var deleteEvent *automationdomain.ScheduledTaskEvent
	for index := range events {
...
func TestDeleteTaskCleansIsolatedAutomationSessions(t *testing.T) {
	...
	now := time.Now().UTC()
	matchingA := protocol.BuildAgentSessionKey("agent-1", "automation", "dm", "scheduled-task:"+task.JobID+":run-a", "")
