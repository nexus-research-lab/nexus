...
func TestRunTaskNowMarksMainEventFailedWhenWakeValidationFails(t *testing.T) {
	...
	if _, execErr := db.Exec(`UPDATE automation_scheduled_tasks SET wake_mode='bad-mode' WHERE job_id=?`, task.JobID); execErr != nil {
	...
	var status string
	row := db.QueryRow(
		`SELECT status FROM automation_system_events WHERE event_type='scheduled_task.trigger' ORDER BY created_at DESC, event_id DESC LIMIT 1`,
	)
