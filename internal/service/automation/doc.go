...
//   - delivery_authority.go：创建 provenance 与独立 delivery grant 分离，以及 create/update/投递/重试时的 owner-main/self/Room/active IM pairing 动态复核。
...
//   - permission_snapshot.go / permission_policy.go / permission_scheduled.go / permission_decision.go：Session/Agent 权限 copy-on-create、独立任务授权策略、运行时拦截、持久决策与安全恢复。
