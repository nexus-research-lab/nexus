...
//   - connection*.go / configuration_state.go：连接查询、刷新、稳定 credentials key_id、
//     active/legacy keyring 存储与脱敏配置版本快照。
//   - mutation.go：owner + Connector 串行化、数据库 CAS 与单调配置版本事务。
//   - oauth_*.go / device_flow.go / device_attempt.go：OAuth 授权、飞书扫码选取/创建应用、
//     带 key identity envelope 的 attempt 级 client 绑定、手工凭据兜底与成功后原子切换。
//   - richmail_pairing.go：固定 loopback RichMail 配对、短期 opaque attempt、配置版本 CAS 与工具发现。
...
//   - custom_mcp_discovery.go：自定义及固定 RichMail MCP 的受限连接与只读 Tools 目录发现；stdio 不由宿主执行。
//
