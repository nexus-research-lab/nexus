// INPUT: 已上线 IM 平台能力、默认端点与用户可配置凭据字段。
// OUTPUT: 频道目录元数据、扫码能力与手工配置入口。
// POS: channels/management 的目录真相源，不执行平台连接或凭据持久化。
package management
	...
	items := []ChannelCatalogItem{
		{
			...
			RuntimeStatus:  "ready",
			RuntimeNote:    "使用钉钉扫码一键创建机器人并通过官方 Stream 模式接收入站消息；也可手工填写应用凭据作为兜底。",
			SupportsGroup:  true,
			SupportsQRCode: true,
			CredentialFields: []ChannelCredentialField{
				{Key: "client_id", Label: "Client ID（手工配置可选）", Kind: "text", Placeholder: "扫码会自动填写；也可手工输入 AppKey"},
				{Key: "client_secret", Label: "Client Secret（手工配置可选）", Kind: "password", Secret: true, Placeholder: "扫码会自动保存；也可手工输入 AppSecret"},
		...
		},
		{
			...
			RuntimeStatus:  "ready",
			RuntimeNote:    "使用企业微信扫码绑定智能机器人并建立长连接；也可手工填写 Bot ID 和 Secret 作为兜底。",
			SupportsGroup:  true,
			SupportsQRCode: true,
			CredentialFields: []ChannelCredentialField{
				{Key: "bot_id", Label: "Bot ID（手工配置可选）", Kind: "text", Placeholder: "扫码会自动填写；也可手工输入 Bot ID"},
				{Key: "secret", Label: "Secret（手工配置可选）", Kind: "password", Secret: true, Placeholder: "扫码会自动保存；也可手工输入 Secret"},
		...
		},
		{
			...
			RuntimeStatus:  "ready",
			RuntimeNote:    "飞书扫码页可选择已有应用或创建新应用并授权；也可手工填写已有 App ID / Secret，默认通过长连接接收入站消息。",
			SupportsGroup:  true,
			SupportsQRCode: true,
			CredentialFields: []ChannelCredentialField{
				{Key: "app_id", Label: "App ID（手工配置可选）", Kind: "text", Placeholder: "扫码会自动填写；也可手工输入 cli_xxxxxxxxx"},
				{Key: "app_secret", Label: "App Secret（手工配置可选）", Kind: "password", Secret: true, Placeholder: "扫码会自动保存；也可手工输入 App Secret"},
