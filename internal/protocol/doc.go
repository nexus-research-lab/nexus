...
//
// 成员地图：
//   - agent.go / agent_private.go / skill.go：Agent、同 owner 联系人、受控执行工具策略与 Skill 协议。
//   - session*.go / conversation_turn.go / input_queue.go：会话、消息、轮次、
//     外部 IM 身份、Goal 完成收据、上下文占用和持久输入队列。
//   - room*.go：Room、联系人内部通道、成员 participation gate、唯一 conversation draft、
//     directed message、public handoff 与 runtime slot。
...
//     command catalog、interrupt ACK、owner/session WorkGraph 失效事件与前端 TS 生成。
