...
- 记忆工作面保留目录与正文分栏，底色继承 Agent 详情容器，与工具、技能等同级栏目一致，不另加目录底色、正文混色或装饰阴影。目录工具栏保持一排：搜索占剩余宽度，刷新嵌入公共搜索框尾部，右侧使用紧凑的标准具名类型筛选；真空目录与无匹配结果只显示其中一个，后者可清除当前筛选。分组名称与更新时间使用可读的共享元数据排版，不全大写或使用微小弱色文字。正文摘要标题允许换行，写入状态放在元信息层，操作按可用宽度换行；访问失败仍保留窄栏返回目录入口。冲突对照由正文容器宽度决定：不足 640px 为单列，其余为双列，不能按全窗口宽度挤压两份文档。
