...
- 记忆页按用户选择保留改版前的整套布局：同行搜索（尾部刷新）与仅显示当前值的紧凑类型菜单，原目录行、分组字级、单行正文标题、更新时间与状态排列，以及原有软分栏、混色和阴影；单页与正文不再分别套用后续试改。它继续组合公共 SearchInput、SelectMenu、ListRow、Button 与 SourceEditor，不新建私有控件。具名加载、互斥空状态、访问失败返回及保存/冲突对账保持有效。
