...
- 侧栏折叠后，展开开关必须脱离共享 Header 裁剪壳；Web 中与 48px Dock 共用水平中心轴，macOS 中固定在窗口控制按钮右侧并与其共用垂直中心轴。侧栏外缘竖线从 Header 底部开始，禁止穿入系统按钮区域；正文保持原有内容轴，只有位于原生标题栏平面的共享 Header 增加横向安全区，位置变化不应引起 Dock 或正文纵向跳动。
