...
- Agent 选择文字统一由 `lib/agent-selection-options.ts` 生成。普通唯一姓名保持原样；同名/缺名项使用前置显示序号，避免窄控件截断时丢失区分信息。序号按当前完整目录的创建时间、同时间或缺时间时的稳定 ID 顺序生成，只是局部展示辅助，目录成员/姓名变化时允许更新，不能作为持久名称或身份；成员筛选必须复用同一完整目录上下文。当前绑定不在候选中时保留其 value 并显示禁用的“当前智能体不可用”，空选项/默认房主仍是另一种状态；不得自动换成第一项。Pairing 创建在当前 Agent 不可用时阻止提交，其他领域的资格与写入校验继续由原所有者负责，不以目录显示策略替代服务端身份验证。
