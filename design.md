...
- 记忆目录单页继承 Agent 详情容器底色，与工具、技能等同级栏目一致；单页工具栏保持搜索、内嵌刷新与紧凑类型筛选同排。进入单篇记忆后保留原有正文混色与羽化阴影，宽容器继续使用目录/正文软分栏；分栏中的窄目录将搜索与类型筛选分行，避免压缩输入空间；真空目录与无匹配结果只显示其中一个，后者可清除当前筛选。分组名称与更新时间使用可读的共享元数据排版，不全大写或使用微小弱色文字。正文摘要标题允许换行，写入状态放在元信息层，操作按可用宽度换行；访问失败仍保留窄栏返回目录入口。冲突对照由正文容器宽度决定：不足 640px 为单列，其余为双列，不能按全窗口宽度挤压两份文档。
