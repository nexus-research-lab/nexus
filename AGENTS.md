...
- 模型通过 Goal command 创建的 Goal 必须把经服务端验证的当前 Agent 持久化为负责人；后续新物理 round 只为该负责人解析启动时的 exact Goal/objective revision，并且该快照只进入 round-scoped Goal command authority，不得泄漏为 ambient Execution/WorkGraph authority。exact Room conversation 的协作者只能读 Goal、共享 WorkGraph 的目标/拓扑/状态和交付证据；只读观察不得授予 Assignment、Review、Submission、Plan mutation、Goal 或 coordination capability。旧 round、旧 revision、后台/外部来源仍必须 fail closed。
