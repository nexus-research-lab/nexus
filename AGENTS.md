...
- Runtime Graph 必须复用 tool policy 的严格单进程语法识别 Goal/Execution CLI transport，只持久化分类而不持久化 raw command/input；这些控制调用始终作为 direct owner 的 `detail` 审计事实，不因 Bash 名称、失败、重试、Artifact 或控制边进入主画布。Work Item/Attempt/Review/Gate 继续表达权威图变化，普通 Bash 与外部可观察动作保持可见。
