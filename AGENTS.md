...
- 普通 HTTP 失败可显式携带开放 `FailureCore v1`，但旧 `WriteFailure` 与各领域既有身份默认不变。`failure.transport_request_id` 只复用当前 `X-Request-ID` 做诊断，不得变成幂等、授权、路由、缓存或业务身份；Conversation `client_request_id/client_message_id`、Configuration/Automation/Goal/Execution 的既有 `request_id` 与 run/round/resource identity 继续各自拥有真相。数据影响只能由事务、revision、durable ACK 或领域回执证明；断线/超时必须进入 unknown 并先对账，禁止自动重放副作用。完整合同见 `docs/specs/failure-recovery-spec.md`。
