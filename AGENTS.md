...
- 普通 HTTP 失败可显式携带开放 `FailureCore v1`，但旧 `WriteFailure` 与各领域既有身份默认不变。`failure.transport_request_id` 只复用经过限长和字符校验的当前 `X-Request-ID` 做诊断，不得变成幂等、授权、路由、缓存或业务身份；Conversation `client_request_id/client_message_id`、Configuration/Automation/Goal/Execution 的既有 `request_id` 与 run/round/resource identity 继续各自拥有真相。数据影响只能由事务、revision、durable ACK 或领域回执证明；断线/超时必须进入 unknown 并先对账，禁止自动重放副作用。结果未知的同一修改目标只能由精确领域读取或用户明确的新意图解除，后台刷新和不相关读取不得解锁；读取辅助数据失败不能摧毁仍有权限的主快照。完整合同见 `docs/specs/failure-recovery-spec.md`。
