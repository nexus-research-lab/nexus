...
# 需要忽略的目录名，与 WorkspaceFileManager.list_files 保持一致
_HIDDEN_DIRS: Set[str] = {".agents", ".git", "__pycache__", ".claude", ".DS_Store"}


def _should_ignore(path: str) -> bool:
    """判断路径是否属于隐藏/运行时目录，应被忽略。"""
    parts = Path(path).parts
    return any(part in _HIDDEN_DIRS for part in parts)
...
class WorkspaceObserver:
    ...
    async def _compute_diff_stats(before_content: str, after_content: str):
        ...
        return await loop.run_in_executor(
            None, WorkspaceFileManager.build_diff_stats, before_content, after_content
        )
