...
`nexus-control` 仓库默认位于本仓同级目录；其他位置可通过 `NEXUS_CONTROL_BUILD_CONTEXT` 指定。Nginx 将同一 Web Origin 下的 `/auth/v1/*` 转发到 Control，将 `/nexus/v1/*` 转发到 Nexus Server；前者承接账号、成员、套餐和 entitlement，后者承接 token 用量与运行时执行。Control 私有目录使用独立容器 UID 1002，Nexus 进程只读取公开签名公钥。已有 Web 用户切换前按 [Control 迁移文档](../docs/operations/control-migration.md) 停止旧认证写入并导入账号与订阅数据。
