...
### Fixed
- Made the managed Goal/Execution Skill binding a launch-time and Agent read-model invariant instead of a one-shot migration assumption, added a dedicated repair migration, removed stale managed disables before Claude tool-policy projection, made CLI contracts advertise the exact next command, clarified that `goal_binding` belongs to the outer command JSON rather than the Plan YAML, and made all managed command Skills read the pre-created input slot before their first write instead of producing a recoverable tool failure.
