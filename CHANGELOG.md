...
### Fixed
- Distinguished owner-level Connector configuration/authorization from current-Session selection in the DM runtime context, so Agents no longer restart OAuth for an already connected Connector or misreport a selected-but-missing schema as an authorization problem.
