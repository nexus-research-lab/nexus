...
- Reduced always-on Agent and Room prompt repetition: removed duplicate Goal/Execution guidance, kept Connector tool-surface facts in the Session system prompt instead of user turns, and left lifecycle and command detail to managed Skills, round-scoped CLIs, and execution context while preserving Nexus identity, routing, and Room public/private boundaries.
...
- Made connected marketplace MCPs opt-in: Agents default to no attached Connectors, Agent settings can define defaults, and the Composer can override them for the current Session. Runtime guidance now distinguishes account configuration from Session selection, points users to the Composer's left-side `+` menu, and grounds attached managed Connector tools in current-turn facts. DM Connector changes now coalesce into a latest-wins background fork so nxs can prepare the new tool surface before the next message, while send-time fork remains the fallback for runtimes that publish identity only after a query.
...
- Simplified scheduled-task creation to independent versus reused Agent context, an optional real Nexus/Room/IM destination session, and stored versus delivered results. Execution and delivery can use different same-owner Agents; synthetic task inboxes and script creation/editing are no longer shown, while historical script and inbox tasks remain readable and runnable for compatibility.
...
- Kept each immutable WorkGraph Submission and its Acceptance on one review Gate even after the current Assignment lease is cleared, and aligned control-return lines with the same target center anchor used by normal process flow while retaining their outer U-shaped avoidance route.
...
- Restored the retired MCP input boundary on the Goal/Execution CLI by validating portable operation schemas before domain reads, requiring a fresh exact contract for each new round-private input write, preserving exact bound Room worker/reviewer reads, and keeping authoritative responsibility context instead of fabricating refresh state when results are large.
- Distinguished real shell environment overrides from ordinary `NAME=value` text in workspace isolation while directing managed Skills to call the injected CLI without environment probes, eliminating misleading preflight failures without weakening capability protection.
...
- Replaced per-command source builds of the Agent-facing `nexus` CLI with a startup-refreshed same-version host multicall shim, preventing local `go.work` drift, compiler mismatches, and process startup overhead from breaking Goal or WorkGraph commands.
- Kept the managed Goal/Execution input slot alive for the complete physical round, normalized validated editor replacements back to owner-only permissions, removed the competing inline/caller-file transports, and preserved malformed-input retry limits across separate CLI processes.
- Removed the fixed five-second outer deadline from runtime replacement cleanup so the SDK owns graceful disconnect timing instead of surfacing immediate `context deadline exceeded` rounds.
- Stabilized Room Agent message identity and streaming message height so a newly started collaborator no longer disappears until refresh or makes the conversation jump vertically while tokens arrive.
- Kept strictly recognized Goal/Execution Bash and PowerShell CLI transport calls—and exact historical Goal/Execution MCP rows—in the owning Agent's Runtime Graph details instead of rendering them as WorkGraph canvas nodes. Ordinary successful shell commands now stay in details unless an active/abnormal run, artifacts, an explicit visibility hint, retry/loop-back structure, or recovery makes them important; Review and loop-back edges stay attached to structured Work Item/Agent and Gate facts rather than hidden `submit_work` transport.
...
- Made the managed Goal/Execution Skill binding a launch-time and Agent read-model invariant instead of a one-shot migration assumption, added a dedicated repair migration, removed stale managed disables before Claude tool-policy projection, made CLI contracts advertise the exact next command, clarified that `goal_binding` belongs to the outer command JSON rather than the Plan YAML, and made all managed command Skills read the pre-created input slot before their first write. Managed input reads now safely restore editor-replaced files to `0600` after link/race validation, so models no longer produce recoverable Write or permission failures or run `chmod` themselves.
- Made “Run again” replace runtime-startup failures that were durably accepted before any SDK transcript existed, while preserving strict UUID-bound transcript pruning for ordinary edits and completed rounds.
- Restored managed Goal/Execution commands for upgraded Agents and Room rounds by making both Skills system-locked, migrating stale Execution bindings and disables, rehydrating canonical Agent runtime profiles, auto-approving both semantic Skills, and granting only the exact per-round CLI input directory.
- Distinguished owner-level Connector configuration/authorization from current-Session selection in the DM runtime context, made the host snapshot authoritative over stale turn conclusions and direct CLI rechecks, and gave tool-surface retirement enough time for both graceful and forced process shutdown so Agents no longer restart OAuth or fail a valid remount at the five-second boundary.
- Forked the current DM transcript into a new physical nxs or Claude Code session whenever an explicit Connector changes the model-visible tool surface, retiring the warm client first and committing the new SDK identity only after its transcript is recoverable. Claude Code may publish that identity from the first-query init event instead of `Connect`; the host now keeps the fork pending until that event and rejects/cleans any source identity. This replaces the K3-only cold reset and keeps the selected Feishu/GitHub MCP schemas, logical Nexus Session, title, and visible history aligned.
- Made conversation forks durable before workspace materialization, protected pending branches from source-transcript deletion, transferred delayed transcript cleanup ownership after SDK materialization, and compensated failed forks even after request cancellation.
- Replaced full-scan history pagination and per-open Round Navigator scans with one host-side SQLite/B-Tree read model for DM and Room. Cold legacy sessions now return a short retryable indexing state while one bounded rebuild continues across request cancellation; warm reads validate canonical sources and load only navigation metadata, bounded cursor windows, and selected round payloads, with atomic generations, corruption recovery, retention, and deletion fencing.
...
- Replaced the one-second Execution, Subagent, and Automation delivery scans plus the three independent 15-second orchestration recovery scans and 10-second Goal scan with durable startup recovery, coalesced mutation wakes, exact retry/lease timers, and bounded low-frequency audits; Automation leader leases remain independently renewed and PostgreSQL leaders reconcile cross-instance catalog changes.
...
- Kept launcher and sidebar directory loading independent of full conversation history, distinguished initial failure from a real empty directory, retained stale directory data on refresh failure, coalesced realtime invalidations without request starvation, and stopped benign ResizeObserver warnings from polluting desktop fatal diagnostics.
...
- Unified the mutable round-local Goal, Execution, coordinator, worker, and reviewer authority state so accepted review, Goal retarget, successor planning, assignment, and delivery can advance safely in one physical round; stable `submit_work` and `review_work` schemas now distinguish visible assignment projections from exact host-issued bindings, exact bindings supply omitted locators, and unbound or explicit mismatched calls fail closed with actionable guidance.
- Hardened Goal lifecycle controls with exact-round pause interruption, durable blocker identity and recovery context, continuation-ready scheduler pagination that cannot starve behind suspended Goals, per-Goal failure isolation, and same-revision continuation limits that pause/resume cannot reset.
- Made Subagent control and recovery user-operable: orphaned parent-exit reconciliation survives restart, owner/session/task boundaries return fail-closed 404s without runtime side effects, and capability-driven stop, continue, and resume controls now expose explicit confirmation, unsupported, and failure states in the task thread.
- Routed Room `nexus_comms.send_message` calls with no `conversation_id` to the trusted current topic instead of the Room main conversation, required explicit routing across Rooms, preserved exact Goal-revision handoff attribution across queues and restart recovery without granting target capabilities, and stopped successful send/read/list/todo calls from fabricating Goal continuation progress.
- Made Goal continuation launch crash-safe with a server-only durable plan receipt: scheduling atomically stores the full prompt and authority identity with the continuation count, scheduled/claimed/started receipts remain a one-open-plan fence until exact runtime terminal settlement, Room terminal callbacks settle that receipt with the outer/root round while retaining the distinct Agent round for audit, handoffs can resume immediately instead of waiting for lease recovery, expired claims or registered-runtime receipts recover after restart without recounting, duplicate workers are CAS-fenced, transient runtime-start failures retry with backoff without suspending the Goal, objective/lifecycle changes cancel stale receipts, and unsafe opaque legacy reservations are refunded without erasing already-consumed continuation attempts.
- Made the accepted-review-to-Execution-completion boundary crash-safe with a transactionally persisted completion-audit receipt, startup/event/deadline recovery with bounded audits, legacy active-graph recovery, and CAS-protected terminal settlement without conflating backend readiness with visible alignment evidence.
- Separated all DM/Room Goal entry paths from WorkGraph creation: model and Composer Goals now remain Goal-only until an explicit `goal_binding=current` Plan proposal owns and confirms the binding, while `goal_binding=none` remains Goal-free; automatic Goal continuation now grants one action-oriented recovery turn before suppressing a second consecutive no-progress turn, so inspect-then-retarget flows no longer stop between steps.
...
- Kept same-round Room Goal collaboration on the revision successfully returned by `retarget_goal`, carrying one ledger-frozen attribution through public mentions, directed wakes, progress accounting, queues, and recovery without granting collaborators Goal mutation authority; terminal accepted WorkGraphs can now reserve a fresh Goal revision successor while preserving their terminal history instead of trapping the active Goal in a retarget retry loop.
- Returned Room Goal completion authority to the current lead: member count and non-lead collaboration evidence are now optional audit facts rather than completion gates, while active Room handoffs/queues/wakes and confirmed WorkGraph readiness still prevent an inconsistent close.
...
- Classified late WorkGraph submissions from a Goal-retargeted predecessor as a muted `superseded` outcome instead of a transport failure or semantic rejection, and made the superseded root Attempt's later physical terminal callback idempotent so one expected cancellation no longer cascades into two visible binding errors.
- Fixed editing an existing scheduled task to another Agent with an IM execution or delivery session: the update now persists the selected Agent together with its session routes, validates the exact active pairing against that Agent, and keeps immutable creation provenance from switching the form back to the original creator.
- Fixed scheduled-task delivery to a different Agent's real Nexus or IM Session by routing and projecting under the recipient Agent while preserving the execution Agent as producer metadata; new UI and Automation MCP flows no longer create or offer a synthetic “scheduled task inbox.”
...
- Made Room Lead self-assignment issue an explicit host-only WorkBinding receipt into the active round, so subsequent tools, subagents, and Runtime Graph nodes follow the exact Work Item instead of remaining grouped under the coordinator Agent; DM keeps its separate implicit segmentation path.
...
- Removed the redundant standalone/reserved badge from Goal-only status strips while retaining the server-owned binding state for clear authorization and WorkGraph transitions.
...
- Restored model-created Goal ownership across physical rounds: the verified Room lead or DM Session Agent now receives a private exact start-of-round Goal revision for retarget, alignment, complete, and blocked operations, while collaborators, stale rounds, and the shared Execution MCP remain unbound. Goal Composer submissions are claimed immediately in their original Session; switching conversations no longer cancels an already-sent request, ACK/explicit rejection settles only the locally owned `client_request_id` without projecting into the newly selected Feed, and an unknown acceptance result cannot restore a duplicate Goal draft. Explicit failures still restore only when no newer draft exists.
