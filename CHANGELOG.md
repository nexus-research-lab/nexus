...
### Fixed
- Forked the current DM transcript into a new physical nxs or Claude Code session whenever an explicit Connector changes the model-visible tool surface, retiring the warm client first and committing the new SDK identity only after its transcript is recoverable. This replaces the K3-only cold reset and keeps the selected Feishu/GitHub MCP schemas, logical Nexus Session, title, and visible history aligned.
