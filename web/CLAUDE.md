...
- Light/Sunny 壳层以 `#f9f9f7` 为页面真相源，导航、目录、主画布依靠相邻中性灰阶分区；展开的主侧栏外缘只绘制一根从物理窗口顶端贯穿到底部的不透明 hairline，折叠时侧栏与分界完全退出画布，恢复动作进入原生窗口控制安全区，Surface Header 只让出恢复触点与内容之间的实际间距，不保留独立空栏，内部 Dock 不再叠加竖线或外投影。Nexus 品牌蓝只用于发送、保存、创建、连接等主行动，以及焦点、运行态和明确选中模式；普通导航与次级工具保持黑白灰，teal 只表达次级数据/文件类型，红绿黄只表达危险、成功和警告
...
- Room 当前 Session 的 Agent execution 展示锚点只由 `hooks/agent/runtime/model/room-agent-execution-state.ts` 维护：首次批量恢复采用服务端 `display_order` 或 message/slot timestamp + index 建立 canonical 顺序，后续 permission、增量 slot、stream、status、message fallback 证据统一换算到同一时间尺度，只能追加并接管既有节点；只有 Room 订阅恢复的权威 `pending_snapshot` 可以收口其中缺失的非终态 execution，以中性 `done` 保留 shell/顺序并允许迟到 exact cancelled/error 单调修正，普通空 ACK 不得清理其他 slot。acknowledged tombstone 保持原 shell 但不再携带交互。精确停止由 runtime 易失层按 `agent_round_id` 维护 stopping，并由 terminal event / `interrupt_ack` 幂等收口；Timeline、Room Feed 与 Thread 只消费该投影，不得按完成顺序重排
...
- Composer 由 `features/conversation/shared/composer/controller/` 分离草稿、分阶段消息投递、Goal、Session Connector 与 DM Echo 显式开关、有序键盘守卫，以及输入/运行时/模式/动作视图投影；Echo override 只能由 DM scope 显式开放，Room 即使只有一个成员也不得按 targets 数量猜测。未发送的正文、图片/文件附件、Message/Goal 模式、Room Goal 负责人和 Mention 目标组成单一草稿胶囊，按包含 Session ID 的 Room/DM 内存作用域隔离，切换 Session 恢复各自待发送状态且不同聊天互相隔离；本地协议派发后立即认领并清空原修订草稿，Goal、普通消息、编辑重跑与队列输入都由 durable transport owner 在 Session 切换和新建后继续等待，页面级请求才取消；ACK/拒绝按本地拥有的 `client_request_id` 收口且只在原 Session 投影，发送前失败或后端明确拒绝只在没有新输入时恢复，受理未知绝不回填以免重复发送；Goal 的受理未知另在原 scope 显示确认中，并以提交前 Goal ID/version fence 或原 Session exact `client_message_id` durable 控制记录对账，期间禁止重复提交；textarea 高度只由浏览器真实排版在正文、原生 input/IME 与宽度变化时重测；输入历史仍按不含 Session ID 的逻辑聊天作用域共享；`components/{footer,pending-queue}/` 分别拥有展示和局部交互，面板只装配子域
...
- 锚定浮层共用 `shared/ui/overlay/` 的命名布局 preset、定位、Portal 和关闭生命周期；Feature 只提交 preset、方向、对齐和内容高度估算，不直接传 gap、viewport inset 或 min/max 窗口尺寸；Select/MultiSelect 在 `shared/ui/menu/` 复用内部开关、触发键盘协议和 listbox 框架，ActionMenu 保持外部受控，消费者直接导入具体组件
...
- 对话滚动只通过 `features/conversation/shared/timeline/scroll/` 协调；FOLLOW、READING、Virtualizer、历史前插、轮次导航和局部展开任一几何提交都必须服从单一 `scrollTop` owner，面板不得复制底部阈值、RAF 动画、历史前插锚点或轮次 DOM 标记；Feed 使用真实内容高度，禁止保留历史最大高度制造空白；Composer 的交互高度保护只允许原子释放，禁止连续高度动画形成 ResizeObserver 反馈
...
- 浏览器消息集合按完整 root round 同时受 round 数与估算驻留字节预算约束；淘汰不得拆开 round，around 窗口保留目标锚点，实时/乐观消息优先于可重取的历史。超过阈值的历史 Tool result 与内联图片只在消息页保留当前 generation 引用和有界预览，展开 Tool 或挂载图片时才通过带认证、可取消的 detail 请求读取完整内容
- DM/Room Todo 只从 `features/conversation/shared/todos/` 的单遍轮次投影派生；计划、运行时任务和状态别名不得在面板中重复推导
- 会话导航由 `shared/session-navigator/` 分离时间线数据投影、刻度视觉模型、纯 DOM 定位和活动轮同步，`session-navigator/jump/` 分离 exact 索引目标、串行加载与落点确认；缺失窗口加载必须绑定会话键和请求代次，落点前导航独占滚动且不得提前切换活动态，DOM 确认可见后立即交还 Virtualizer/READING，失效目标不得产生副作用
- 消息项由 `features/conversation/shared/message/item/controller/` 统一完成顺序、权限、过程链和最终回复投影，`controller/display/` 分离纯显示状态、结果元数据结算资格与展开生命周期；Assistant 视图按模型、内容、头部、过程和权限适配分工，受信 `metadata.source=echo` 只在头部显示轻量 Echo 标识，不进入正文。不得根据 streaming 自行隐藏或恢复模型与统计，DM live 的连续普通工具由 `item/process/dm-tool-run-segments.ts` 以首个 `tool_use.id` 稳定成段，未解析或新增长段保持展开，已解析段在叙事/final 恢复边界后折叠，Room 与人工交互工具不进入该压缩路径；User 视图按展示模型、头部、正文和编辑器分工，未匹配权限只进入 Composer 队列，不得在消息正文恢复独立操作面
