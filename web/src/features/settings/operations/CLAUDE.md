...
- 组织角色缺失时显示身份恢复提示，不回退为普通成员或平台角色；目录没有可管理成员时隐藏操作列。
...
运营子页页头保持全宽，正文复用设置共享的 1200px 居中限制，不另设宽度规则。组织成员与项目以目录和状态为主，项目新建表单使用公共 Disclosure 按需展开；成员目录在本地快照上提供姓名和用户名搜索，组织生命周期动作、邀请成员和邀请记录共用工具栏左侧，右侧为搜索；窄屏允许换行，稳定列分离成员、角色、访问状态与操作。成员名、项目 ID 和路径允许完整换行。表单依据自身容器宽度分列，普通输入、选择和提交为 md，目录内动作和角色选择为 sm。装饰图标与项目权限版本不占用主内容层级；实际权限限制与写结果反馈保持可见。
...
- 邀请成员、邀请记录与搜索共用工具栏，不重复展示组织成员标题与数量；目录分离成员、角色、状态、操作四列，窄容器按行重排。不可编辑的角色直接显示文字，本人不显示不可用操作。成员角色控件固定为 104px；唯一的移出组织操作直接显示“移除”文本按钮，成功后隐藏该成员，不停用账号；邀请记录每行仅有撤销或删除一项操作，直接显示文本按钮。邀请记录在独立弹窗中显示总数、待接受数和紧凑条目，不占用成员正文。
