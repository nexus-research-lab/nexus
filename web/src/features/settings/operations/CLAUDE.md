...
- `organization-panel.tsx` 以 Organization 名称作为宽窄屏都必须可见的页面身份，成员目录是主内容，角色说明是辅助内容；邀请参数和一次性链接只在短弹窗中出现，邀请记录降为默认收起的次级区块，撤销保留在对应记录行。
...
所有子视图复用 Settings Group、Control Label、语义 Typography、Badge 与 Resource State；组织概览、成员目录、订阅摘要和项目使用同一弱底分组面，资源目录采用紧凑工具栏与稳定列，不得在页面内重组字号/字重/行高、状态胶囊、加载空态或任意圆角。
...
运营子页共用壳层内容宽度，不单独居中限宽。组织成员与项目以目录和状态为主，项目新建表单使用公共 Disclosure 按需展开；成员目录在本地快照上提供姓名和用户名搜索，工具栏显示当前结果数，稳定列分离成员、角色与访问状态。成员名、项目 ID 和路径允许完整换行。表单依据自身容器宽度分列，普通输入、选择和提交为 md，目录内动作和角色选择为 sm。装饰图标与项目权限版本不占用主内容层级；实际权限限制与写结果反馈保持可见。
成员列表统一使用 UiSelectMenu；列表禁用条件继续遵循当前身份权限。

成员与项目的请求顺序及未知结果恢复分别由 `control-members-panel.test.tsx` 和 `project-admin/use-project-admin.test.tsx` 覆盖；订阅账户刷新仅受在途请求限制，不能被等待核对的禁写状态禁用。
