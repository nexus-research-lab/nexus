...
密码表单的规则/校验提示以实例级 ID 关联全部密码输入，保存时表单声明 aria-busy；不从展示错误推断字段失效，也不改变持久恢复命令。

首次资料加载使用具名 busy status；没有成功资料时不渲染身份/用量/密码分区，失败恢复仍由控制器 Feedback 提供。刷新已有资料不卸载内容。

- `personal-usage-history.tsx` 用当前界面语言格式化精确数量，热力图保留未知日期而不补造历史；图表提供键盘交互和明细表。
