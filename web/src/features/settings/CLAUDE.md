...
- `general/`、`runtime/`、`browser/`、`computer-use/`、`personal/` 与 `provider-settings/` 各自拥有设置资源和交互；`general/` 管理当前用户所有 Agent 共享的 Echo 开关，`browser/` 只在桌面端展示扩展安装、连接状态和完整 CDP 权限，`computer-use/` 独立管理用户开关、固定包和 sidecar，不依赖 Browser。
