...
Provider 目录/详情以自身 720px 工作面宽度作为横向分栏边界，横向服务目录宽 240px，仅保留一层右侧留白，设置和运营嵌入态复用；详情不再叠加额外横向内边距，字段沿统一内容轴排布。
...
服务目录仅有删除操作，使用小号删除图标按钮，鼠标悬停或键盘聚焦该行时显示；无悬停能力的设备保持可见，继续进入原占用检查/确认流程；模型区以分隔线区分配置区，无模型且无搜索词时隐藏搜索框，搜索无结果仍保留搜索入口。私有和公共服务共享该交互。
