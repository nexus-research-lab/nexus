...
默认消息行为复用左侧行标题，分段控件通过 aria-label 命名；默认模型行同样不重复显示右侧“服务 / 模型”标签；排队/打断命令与 Preferences 加载/保存锁仍由原控制器拥有。
...
默认模型行直接合并 Preferences 首次加载与保存锁，目录重试由 providerOptionsLoading 禁用并播报忙碌；权限选择器由共享组件生成实例身份，不使用全局固定 DOM id。
