...
- `view/sidebar-panel.tsx` 用单一常驻壳层处理展开与收起；顶部品牌栏只承载 Launcher 字标与折叠控制，聊天、联系人、能力和固定会话共同对齐导航轨的水平中心轴，轨道尺寸与完整中英文标签要求由 `design.md` 定义，主 Tab 与固定会话必须复用同一 32px 图标框表达选中和 hover；有固定会话时，`sidebar-pinned-conversations.tsx` 仅在展开态于能力下方显示短分割线，继续同一 Dock 几何并支持按落点拖放排序，X 位于右上外沿且只提交取消固定。Nexus 主智能体不再拥有独立 Dock 动作或 Focus 侧栏，而是复用聊天目录行。桌面收起时整个侧栏退到 0 宽并进入 inert，只在工作区顶栏安全区保留展开控制；导航轨、目录、底部操作和外缘 hairline 不得留下占位。聊天、联系人和能力目录的放大触控间距只在 559px 手机单窗断点生效，中等宽度桌面保持紧凑 gutter。手机全宽目录顶部只保留品牌。设置、引导、桌面更新提示与有效退出仅在展开态进入底部操作区；更新提示只在桌面桥接确认有新版本时出现并固定在右下，退出收进账号菜单，帮助保持为最右侧系统动作。展开态复用 `desktop-rail` 的主题底面，让导航、目录和主画布通过相邻中性灰阶分区；只有与主画布相邻的展开侧栏绘制置顶且不透明的全高外缘 hairline，手机全宽目录和独立设置侧栏不得误用。

路由到主 Tab 的映射保持单一来源；业务引导统一由 `features/onboarding/` 提供。

- 底部设置与帮助使用并排的公共圆形图标按钮，帮助固定最右侧，桌面更新提示位于设置左侧。本地模式不显示账号入口；有效密码认证会话保留左侧紧凑头像，点击向上展开带账号名称的退出菜单。账号资料与退出可见性由 controller 投影，菜单复用 UiActionMenu，头像复用 UiAgentAvatar。
