...
- 已登录 Control 远程账户且 Relay 可用时，控制器把当前用户加入的在线 Room 作为普通不可删除条目投影进同一目录，并复用统一的置顶、活动时间与标题排序；本地与在线建群共用同一个创建弹窗，在线类型只在 Relay 可用时出现并额外提供 Team 真人邀请。目录读取与显式建群由 `features/team/` 持有，桌面本地免登录用户不得探测 Relay。
...
- `sidebar-list-rows.tsx` 以头像、元信息、状态和摘要子视图渲染目录行；删除按钮保留元信息列的水平位置，垂直定位以整个列表行为基准，元信息槽不得另设定位容器；桌面与手机的聊天、联系人身份锚点统一使用 40px 头像，手机聊天目录使用 80px 行高，ContactRow 保持 72px 密度且只显示静态 Agent 目录信息，不推导运行态。Room 工作态使用低饱和蓝灰 `running` 语义，待确认使用 warning 徽标；两者都保留头像活动外圈，不复用品牌 CTA 或完成态绿色。
...
- 目录 last_reply_preview 必须先经 message-content-model 的 stripRoomControlMarkers 再进入 Markdown 摘要；不在侧栏重复维护控制标记名单，纯标记结果为空，不改目录原始快照。
