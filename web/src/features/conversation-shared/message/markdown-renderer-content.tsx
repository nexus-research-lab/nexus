...
const FILE_TRIGGER_LABEL_CLASS = "max-w-full break-all whitespace-pre-wrap wrap-anywhere";
...
function createMarkdownComponents(
  ...
  return {
    ...
    p({ children }) {
      return (
        <div className="mb-2 mt-2 min-w-0 max-w-full leading-relaxed text-foreground/90 wrap-anywhere last:mb-0">
          {children}
        </div>
      );
    },
    ...
    li({ children }) {
      return (
        <li className="max-w-full pl-1 leading-relaxed wrap-anywhere [&>p]:m-0 [&>p]:leading-relaxed">
          {children}
        </li>
      );
    },
    blockquote({ children }) {
      return (
        <blockquote className="my-4 w-full min-w-0 max-w-full overflow-hidden rounded-r border-l-4 border-primary/30 bg-primary/5 py-2 pl-4 pr-1 italic text-muted-foreground wrap-anywhere">
    ...
    th({ children }) {
      return <th className="px-2 py-2 font-medium whitespace-normal wrap-anywhere sm:px-4 sm:py-3">{children}</th>;
    },
    td({ children }) {
      return <td className="border-t border-border/60 px-2 py-2 align-top whitespace-normal wrap-anywhere sm:px-4 sm:py-3">{children}</td>;
    },
  };
}
