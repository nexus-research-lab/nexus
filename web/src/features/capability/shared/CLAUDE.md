...
- `capability-page-layout.tsx` 只拥有能力域的页面编排、筛选节奏、目录网格、分区标题、详情页内容轴/二级导航、阅读列/配置侧栏和无品牌身份框；不得解释 Skill、Connector、Channel、Loop、WorkGraph 或定时任务状态。
...
- Skill、Connector、自定义 MCP、Loop 与 WorkGraph 的二级页必须由 `CapabilityDetailPage` 持有内容轴，并由内部唯一的 `CapabilityDetailHeader` 渲染“返回目录 / 当前对象”；业务页不得直接引用 `WorkspaceContentDetailHeader`、复制箭头和间距，目录态 Header/搜索也不得残留在详情路由上。
