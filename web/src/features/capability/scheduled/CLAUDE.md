...
- 创建编辑使用右侧页内工作面；注意事项和运行历史弹窗使用 plain chrome；标题只显示当前动作或任务名，内部 Job/Run/Request 身份只在折叠诊断中出现。
...
- 目录删除确认、命令保护说明与看板/注意事项文案均从当前语言目录投影，计划摘要由 scheduled-formatters 统一接收翻译函数和日期语言；确认目标仍来自当前权威任务及配置版本。

- 常用任务卡片以自身目录容器宽度切换一至三列，按钮高度随完整正文撑开；不得沿用视口分列或固定按钮高度导致页内编辑器打开后文字溢出。
