...
// OUTPUT: 按智能体/群聊分组的可搜索锚定浮层；正文布局保持不变，加载失败可重试，未解析值保留提示。
...
export function TaskDestinationPicker({ kind, data, form, actions }: {
  ...
  useEffect(() => {
    if (isVisible) overlayRef.current?.querySelector<HTMLInputElement>("input")?.focus();
  ...
  return <>
    <UiButton ref={anchorRef} aria-label={label} aria-expanded={open} aria-haspopup="dialog"
      ...
      <span className="flex min-w-0 items-center gap-2">
        <span className="truncate">{currentLabel}</span><ChevronDown className="h-4 w-4 shrink-0" />
      </span>
    </UiButton>
    {open && portalContainer ? createPortal(
      <div ref={overlayRef} id={overlayId} role="dialog" aria-label={label}
        ...
        className={`${OVERLAY_SURFACE_CLASS_NAME} fixed z-50 flex flex-col overflow-hidden p-2 shadow-lg`}>
    ...
    {!execution ? <UiButton className="w-full shrink-0 justify-between" variant="ghost" size="sm" onClick={() => select(null)}>
      {t("capability.scheduled_dialog_reply_none")}{currentValue === "none" ? <Check className="h-4 w-4" /> : null}
    </UiButton> : null}
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(100px,35%)_minmax(0,1fr)] overflow-hidden" role="group" aria-label={label}>
      ...
      <div className="soft-scrollbar min-h-0 overflow-y-auto overscroll-contain py-2 pl-2">
        {visibleItems.map((option) => <UiButton key={option.value} className="w-full justify-between gap-2 text-left" variant="ghost" size="sm"
          ...
          {currentValue === option.value ? <Check className="h-4 w-4 shrink-0" /> : null}
        </UiButton>)}
      </div>
    </div>
    <div className="shrink-0">
      {resources.some((resource) => resource.loading) ? <p role="status" className="p-3 text-sm text-(--text-muted)">{t("common.loading")}</p> : null}
      {resources.some((resource) => resource.error) ? <div role="status" className="p-2">
        <p className="text-sm text-(--text-muted)">{t("capability.scheduled_dialog_resource_load_title")}</p>
      ...
      {!filtered.length && !resources.some((resource) => resource.loading) ? <p className="p-3 text-sm text-(--text-muted)">{t("capability.scheduled_no_matching_chats")}</p> : null}
    </div>
