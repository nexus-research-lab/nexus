...
- `skill-detail-view.tsx` 只渲染模型、Agent 使用矩阵和触发命令，不直接调用 API、维护 Effect 或复制市场反馈；使用矩阵保留系统托管、主智能体、独立开关和不可配置等行为差异，名称、状态与说明共同支持决策。
- 窄屏返回语义由应用页面 Header 提供，详情内部面包屑只在桌面显示，并复用共享二级 Header 与原生窗口控件中线对齐；正文保持受控阅读宽度。
