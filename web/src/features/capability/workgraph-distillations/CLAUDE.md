...
- `workgraph-distillation-detail.tsx` 只接收单个 `WorkGraphWorkflow`、可选资源提示与返回、复制、编辑窄动作；内容轴和二级导航复用 `CapabilityDetailPage`，对象标题、说明和动作对齐复用 `CapabilityDetailIdentity`，目标摘要复用共享 Panel 与 Typography。
