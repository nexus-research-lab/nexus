...
- `workgraph-distillation-detail.tsx` 只接收单个 `WorkGraphWorkflow`、可选资源提示与返回、复制、编辑窄动作；内容轴和二级导航复用 `CapabilityDetailPage`，对象标题、说明、目标摘要和动作必须复用共享 Typography、Panel 与 Button。
