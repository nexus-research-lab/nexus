...
- 固定 Connector 与 Custom MCP 共用的工具区在窄窗必须把标题说明与动作分行，动作保持共享 Button 的完整命中区和不换行文本；工具名称、参数与资源状态不得另建私有排版或圆角；外部标题、工具标识、参数与说明的连续长文本允许在面板内换行，不截改服务端原文。
...
- MCP 工具目录与读取错误绑定精确配置身份；切换目标/连接配置立即隐藏旧快照，迟到响应不得覆盖当前目录。同配置刷新才可保留最后成功快照。
