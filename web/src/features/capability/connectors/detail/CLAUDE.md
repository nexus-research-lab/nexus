...
- RichMail 未连接时优先显示 `UiPanel` card 中的三步准备说明，铺满可用内容宽度，使用 supporting 排版并自然换行；连接事实使用共享 `UiDisclosure` 默认折叠，字段按单列对齐，不显示待获取的服务器、协议或 Token 占位。工具区只在已连接时出现，继续保留加载、空态、失败与重试；没有服务端有效期事实时不显示固定 Token 到期天数。
...
- 同身份刷新期间保留详情正文并暂停写动作；访问失败优先于旧详情。固定 Connector 工具读取被拒绝后丢弃快照，RichMail 连接事实同样不得展示被拒绝的目录元数据。
