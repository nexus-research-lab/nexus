...
- RichMail 连接准备说明只保留一个可执行的主标题，阅读步骤使用公共 supporting、路径分组使用 metadata，不再重复顶部小标题或以计数字号承载说明。
- 能力详情弹窗以能力名和正文为主，不重复 Connector 副标题或成功图标；能力项使用普通列表，OAuth scopes 通过共享 `UiDisclosure` 默认折叠为技术信息。
