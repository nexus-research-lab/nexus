...
- 启动登录要求已有有效 Channel 配置；保存配置与重新拉起扫码共享同一控制版本和登录生命周期，不能留下脱离配置的孤立登录会话。
...
- 启动、验证码提交与只读核对直接返回 succeeded 时均刷新频道账号快照；该刷新失败只保留已提交反馈并提供只读重试，不重放登录或验证码。验证码输入具备可访问名称和提示关联，提交/未知结果期间禁用编辑。
