...
- 常态工作面的目录、正文与 8px 分栏间距均继承 Agent 详情容器底色，和工具、技能等同级栏目一致；不另加目录底色、正文混色或装饰阴影，也不用重复摘要、文件路径或装饰性硬线制造层级。标题、提示、索引和正文统一使用 `nexus-memory-document-content` 阅读轴，目录激活态复用侧栏选择样式，仅异常和编辑状态可以使用强调边界。
