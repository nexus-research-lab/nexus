...
- Markdown 正文的文件索引和图片预览由消费侧绑定文档 `agentId` 后注入共享组件，不能回退其他当前选中的 Agent。
- Header 只展示统一投影的可换行摘要标题、更新时间、运行时状态和真实操作，真实路径只作为标题悬浮说明；标题与元数据复用 Typography，动作可随宽度换行。正文记忆提供确认删除，索引不提供删除。标题、提示和正文共用阅读轴，索引导航只展示标题与摘要，路径仅作为内部导航目标。
- Memory 过期提示使用 `UiInlineNotice`，读取、保存和冲突仍按各自资源状态呈现；Panel 只判断事实，不复制提示条的圆角、背景或字号。
- Header 的运行时写入、保存和删除使用共享 Spinner；正文初始加载由具名 ResourceState 持有，访问失败仍保留窄栏返回目录入口，不显示受限正文或编辑动作。
- 冲突对照的列数由 `nexus-memory-document` 实际容器宽度决定，不使用全窗口断点；草稿和已保存版本的读写能力与显式采用/覆盖决策保持独立。
- 普通编辑、冲突草稿和只读版本统一使用 `UiSourceEditor`；Panel 只设置阅读轴、布局和是否只读，不再维护三份原生 textarea 或清除键盘焦点样式。
