...
- 正文 Header 与背景保留改版前布局；普通编辑、冲突草稿与已保存版本继续复用 `UiSourceEditor`，只读版本仍允许选取复制。初始等待保持具名 ResourceState，访问失败保留窄栏返回目录入口。

正文是否已有读取结果以 revision 或正文事实判断，空正文不等于未加载；刷新等待/失败时保留空文件的编辑草稿与 stale 提示。冲突覆盖保存期间禁用“采用最新版本”，防止丢弃正在提交的草稿；双版本布局按正文容器宽度分列。标题、时间和运行中提示由共享 Typography 角色持有。
