...
- `agent-memory-catalog.tsx` 只遍历 Catalog 投影，不重新解释快照、筛选或文档类型规则；搜索与刷新复用 `SidebarSearchField / SidebarSearchAction`，类型筛选在下一行复用 `UiFilterSelect`。目录项复用 `UiListRow density="dense"`，以可读摘要为主标题，它与文档名不同时保留文档名作为次级身份，完整路径只参与搜索与悬停说明。
- 真空目录和无匹配结果互斥，均复用 `UiResourceState`；无匹配时的清除动作只重置当前 query/type，不触发读取或写入。分组与截断说明使用共享 Typography，不维护私有弱色微标签。
- 刷新按钮仅在真实刷新期间使用共享 Spinner 动效，尺寸服从 SidebarSearchAction；静止状态继续显示普通刷新图标，Catalog 不拥有旋转或 reduced-motion 样式。
