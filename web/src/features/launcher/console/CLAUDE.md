...
- Header 灯组只向公共 LottiePlayer 传动画源与外部布局；播放器独占播放/静态偏好与销毁生命周期，Console 不持有播放实例或备用样式参数。
...
顶部 Header 的横向灯组保持源动画 `356:41` 的比例：Web 使用 `139×16px` 保留识别度，桌面宿主收敛为 `104×12px`，Web 灯组左缘对齐下一行品牌图标起点，桌面端对齐文字起点，macOS 灯组置于系统窗口控制下方，避免进入按钮命中区；品牌图标与字标独占下一行，不与窗口控制共享基线。Striper 字款同样按环境分离：Web 保持原始紧凑字距，桌面宿主使用 `0.1em` 加宽字距，两者都使用字体文件真实的 Regular 字重，不合成伪粗体。

- 无可用Provider时提交只打开配置并保留查询草稿，不能返回已受理让输入清空；配置完成才交接同一草稿。当前挂载内记录已提示状态，即使sessionStorage不可用也不循环自动弹出。Mention类型、缺省目录名称随当前语言投影，稳定身份与查询保持不变。
