...
顶部 Header 的横向灯组保持源动画 `356:41` 的比例：Web 使用 `139×16px` 保留识别度，桌面宿主收敛为 `104×12px`，灯组左缘对齐下一行文字起点，macOS 灯组置于系统窗口控制下方，避免进入按钮命中区；品牌图标与字标独占下一行，不与窗口控制共享基线。Striper 字款同样按环境分离：Web 保持原始紧凑字距，桌面宿主使用 `0.1em` 加宽字距，两者都使用字体文件真实的 Regular 字重，不合成伪粗体。
