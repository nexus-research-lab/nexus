...
- `assistant-message-model.ts` 负责 Assistant 快照规范化、同消息合并、内容块身份和终态失败身份提取；内容专有变化与状态变化同等重要，不得因 `stream_status` 未变而丢弃。Provider 错误原文从最终正文去重，当前可靠性投影只返回 exact round/Agent round 与分类，不恢复旧的会话错误字符串扫描入口。
