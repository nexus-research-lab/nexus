...
- 恢复窗口焦点时重新读取身份，让其他设备上的组织移交、退出与解散反映到当前窗口；同一 owner 的组织变化不重置本地缓存，只由 Team 组织作用域清理在线资源。
...
- Auth Provider回归覆盖先撤下旧owner再等待配置、同owner读取失败保留已有快照；不改登录/登出或自动重试协议。
