...
- `router/` 与 `layout/` 分别拥有路由树和常驻应用壳层。认证守卫使用公共资源状态与双语文案，读取失败只展示恢复操作，不显示底层诊断；认证状态仍由 Auth Provider 唯一持有。
- 路由模板与资源身份编码统一消费 `shared/navigation/route-paths.ts`；App 不拥有供底层 Feature 反向依赖的路径常量。
- 路由 Suspense 与首次认证加载占位显式携带 `data-bootstrap-pending`，由 Bootstrap 观察首屏提交后统一通知桌面就绪。
