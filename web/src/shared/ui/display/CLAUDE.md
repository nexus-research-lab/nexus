...
- `UiStateBlock` 的标题、说明和图标承载体只能选择 App Typography 与语义 shape role；小号中性状态（加载/空态等）使用 supporting 常规字重与 muted 色、36px 图标框和 12px 图文间距；中大号空态保留对象标题层级，紧凑错误/决策态使用分区标题层级，不在业务调用方重写字号和圆角。状态根允许长路径/标识换行，标题、说明和动作均受可用宽度约束；图标是装饰，不形成重复名称。ResourceState 保留动作 busy/disabled 与显式 tone，动作排布不能把长恢复文案撑出状态面。
...
- Room 主头像先复制成员集合并按稳定 id 排序再取最多九人；不同目录/详情接口顺序不能改变同一群聊拼图。排序只用于头像投影，不修改调用方业务顺序；Header 与侧栏共享默认上限。
