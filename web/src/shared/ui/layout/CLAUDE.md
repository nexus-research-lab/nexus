...
- 本目录只保留可跨页面复用的加载、Workspace 布局模型、预览视口配方和面板拖拽入口。
- `preview-viewport-styles.ts` 独占只读预览的有限空间滚动、overscroll 与内嵌键盘焦点；工作区源码、表格和 Composer 文本附件复用同一配方，各自拥有内容排版和具名 region。它不复制内容、接管方向键或宣称完整资源已在 DOM 中；源码等宽排版仍归 `form/source-text-styles.ts`。
...
- `workspace-content-header.tsx` 是管理页正文标题、单句说明、页面动作与二级导航的唯一 Header；桌面态常规标题、说明和动作对齐共享顶栏，浏览器以 60px 为最小高度；长标题/说明允许自然增高、动作可换行，不得越过底部分隔线；详情面包屑保持固定导航高度。macOS 由原生红灯 Y 中心推导高度，底部分隔线与侧栏品牌栏贯通。共享顶部补偿必须让 Web 标题与二级导航对齐侧栏品牌栏、让 macOS 内容避开并对齐原生控件；统一标题栏还必须参与窗口手势，并在侧栏折叠或无侧栏时通过共享横向安全区避开 traffic lights 与侧栏展开入口，不得给具体页面添加私有 padding。
