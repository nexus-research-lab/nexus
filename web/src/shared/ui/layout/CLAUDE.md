...
- `preview-viewport-styles.ts` 独占只读预览的有限空间滚动、overscroll 与内嵌键盘焦点；源码和表格复用同一配方，各自拥有内容排版和具名 region。它不复制内容、接管方向键或宣称完整资源已在 DOM 中；源码等宽排版仍归 `form/source-text-styles.ts`。
