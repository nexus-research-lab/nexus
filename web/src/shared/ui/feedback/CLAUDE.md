...
- `feedback-banner.tsx` 自然排版正文，不渲染审计标签或分段标签，并复用 `surface-popover + surface-radius-md`、App Typography 与共享 Icon Button，不自行定义阴影、材质、圆角或字号。紧急程度独立于 tone，默认礼貌播报，只有业务明确要求立即打断时才使用 assertive。
- `feedback-banner-viewport.tsx` 提供固定定位边界、窄屏重排和 `feedback` 语义 layer；当前产品只有单条反馈，不维护伪堆叠协议。
- `inline-notice.tsx` 是页面内容流和 Conversation 状态层的唯一紧凑提示骨架；`contained / edge` 只表达容器关系，tone 只表达图标与轻量背景语义，标题、影响说明和至多一个恢复动作统一复用 Typography 与 `UiButton`。业务页面不得再手写提示条圆角、背景或动作状态。
