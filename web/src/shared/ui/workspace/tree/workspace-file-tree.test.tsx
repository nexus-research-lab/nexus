...
describe("WorkspaceFileTree", () => {
  ...
  it("keeps keyboard rename and delete independent of directory navigation", async () => {
    ...
    await user.tab();
    expect(document.activeElement).toBe(screen.getByRole("button", { name: "common.more_actions docs" }));
    await user.keyboard("{Enter}");
    expect(document.activeElement).toBe(await screen.findByRole("menuitem", { name: "home.rename" }));
    await user.keyboard("{Enter}");
    expect(actions.onRenameEntry).toHaveBeenCalledWith(entries[0]);
    const more = screen.getByRole("button", { name: "common.more_actions docs" });
  ...
  it("routes nested context menus and same-name actions to the exact entry only", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "common.more_actions docs/readme.md" }));
    await user.click(await screen.findByRole("menuitem", { name: "home.rename" }));
