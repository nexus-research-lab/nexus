...
- Workspace Header 主标题使用 `pageTitle` 并保留完整 title；视图条由 UiTabs 提供 metadata 排版，窄窗选择采用 UiSelectMenu 的 sm/supporting 档，普通工具沿公共 Button 档位，不重复写字体像素值。调用方只提供文本、节点和明确的 identity/action 语义。
