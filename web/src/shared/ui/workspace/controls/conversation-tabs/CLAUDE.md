...
- 首次进入 Room 只打开恢复目标这一个会话；此后仅由历史显式选择、新建或外部目标导航追加标签，服务端新增历史不得自动补入。打开集合、创建时间顺序与活动项由 `store/room-navigation.ts` 跨 Room 切换和应用重启持久化，关闭项保持关闭；恢复时过滤已失效会话，全部失效才回退到当前最新会话。打开标签共同占满可用轨道，每新增一个标签都按活动态权重平滑缩窄；达到可读最小宽度后，不再硬挤或移除旧标签，而是进入可由触控板、鼠标拖拽与滚动轨道直接浏览的横向溢出区。溢出宽度以活动标签为锚重新计算一屏可完整容纳的普通标签数量，滚动结束时吸附到标签边界，尽量避免可视区停在半个标签上。视觉遵循无阴影的轻量导航层级：当前标签用中性浅底、状态点和中等字重与工作面区分，非当前标签透明，只有连续非活动标签之间保留短竖线，关闭动作只在悬停或键盘聚焦时出现。历史入口固定在导航带左端并承载选择、重命名、删除与分页的唯一会话清单；创建入口固定在右端，以无文字 `+` 图标呈现。滚动轨道只在真实溢出时出现。会话正文由上层仅挂载当前活动会话。
