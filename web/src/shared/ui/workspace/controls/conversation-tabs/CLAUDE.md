...
- `use-conversation-tabs-scroll.ts` 维护标签带溢出测量、活动标签归位、宽度动画稳定后的二次边界校正、触控板滚动和鼠标拖拽；滚轮入口必须以非 passive 原生监听显式归一 `deltaX`/`deltaY`，不得依赖 WebView 的默认横向滚动；Ctrl/Meta 缩放修饰及已消费事件不得拦截，无溢出或已到滚动边界时交还浏览器默认行为。
