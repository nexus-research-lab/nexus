...
- Room 根路由按“活动项、其余已打开项”的顺序恢复持久化标签栏；显式 Conversation 路由优先，非空快照的失效活动项先回退到仍有效的打开标签，全部失效才使用当前资源顺序首项。`active=null` 的明确空快照不得自动恢复历史会话，页面仍保留 Room/Agent 壳层，仅把 Conversation 与 Session 身份置空。
