...
登录介绍随当前语言即时投影；bootstrap 复用 AppLoadingScreen 的可访问状态。字段与标题使用实例级 useId，提交期间禁用凭证编辑并声明 busy；controller 在 React 状态更新前同步防重复提交，不自动重放失败登录。页面回归覆盖语言切换保留草稿、加载切换和同一事件周期的双提交。
