...
// OUTPUT: 复用 Room Header、阅读轨道、本人消息和 Composer 外观的共享 Room 页面。
        ...
        <ConversationPanelLayout>
          <ConversationPanelViewportArea>
            <ConversationPanelViewport
              ...
              viewport={{ isHistoryLoading: false, scrollRef }}
            >
              <TeamMessageFeed
...
function TeamMessageFeed({
  ...
  if (isLoading || messages.length === 0) {
    return (
      <div className={`${CONVERSATION_CONTENT_LANE_CLASS_NAME} flex h-full min-h-64 items-center justify-center text-sm text-(--text-soft)`}>
    ...
    <li className="nexus-chat-message-section px-0 sm:px-3">
      <div className="flex min-w-0 gap-3">
        <MessageAvatar title={author}>
          <span className="text-sm font-semibold">{author.slice(0, 1).toUpperCase()}</span>
        </MessageAvatar>
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-baseline gap-2">
            <span className="text-sm font-semibold text-(--text-strong)">{author}</span>
