...
ALTER TABLE automation_scheduled_tasks
    ADD COLUMN delivery_grant_json TEXT NOT NULL DEFAULT '{}';

UPDATE automation_scheduled_tasks
SET delivery_grant_json = json_object(
    'kind', COALESCE(NULLIF(TRIM(source_kind), ''), 'system'),
...
WHERE TRIM(delivery_grant_json) IN ('', '{}');

-- +goose Down
ALTER TABLE automation_scheduled_tasks DROP COLUMN delivery_grant_json;
